-- BluefinTecsMerchantServices SDK test runner (generated by @voxgig/sdkgen).
--
...
def main : IO UInt32 := do
  ...
  let offline : SIO Unit := do
    -- mandator_clearing_export_download: offline (test-mode) entity behaviour
    (do
      let seedPath := "../.sdk/test/entity/mandator_clearing_export_download/MandatorClearingExportDownloadTestData.json"
      if !(← System.FilePath.pathExists seedPath) then
        IO.println s!"skip - mandator_clearing_export_download: no seed at {seedPath}"
      else do
        let seed ← SdkJson.jsonRead (← IO.FS.readFile seedPath)
        let tclient ← Sdk.testSdk0 seed
        let existing ← SdkRuntime.gp (← SdkRuntime.gp seed "existing") "mandator_clearing_export_download"
        let ids ← keysof existing
        -- load the first seeded entity by id
        if ids.size > 0 then do
          let wid := ids[0]!
          let m ← newMap #[("id", Value.str wid)]
          let got ← Mandator_clearing_export_download.load tclient m (← emptyMap)
          let gid ← SdkRuntime.gpS got "id"
          if gid == wid then pass s!"mandator_clearing_export_download.load offline (id={wid})"
          else fail s!"mandator_clearing_export_download.load offline: got {gid}, want {wid}"
)
  offline.run ctx
  match liveBase with
