# BluefinTecsMerchantServices Lean SDK



The Lean SDK for the BluefinTecsMerchantServices API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/bluefin-tecs-merchant-services-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "BLUEFIN_TECS_MERCHANT_SERVICES").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Cancel_transaction.create sdk (← emptyMap) (← emptyMap)
...
> **`ratelimit`, `retry`, `timeout` wrap the transport**, so each one wraps
