// BluefinTecsMerchantServices SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/BluefinTecsMerchantServicesSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // BluefinTecsMerchantServicesSDKTests
  ...
  static func errCode(_ e: Error?) -> String {
    (e as? BluefinTecsMerchantServicesError)?.code ?? ""
  }
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: BluefinTecsMerchantServicesSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = BluefinTecsMerchantServicesError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: BluefinTecsMerchantServicesSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return BluefinTecsMerchantServicesError(code, msg, nil)
  }
}
