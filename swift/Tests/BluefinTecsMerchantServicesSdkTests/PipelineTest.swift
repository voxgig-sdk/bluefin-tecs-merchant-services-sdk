...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinTecsMerchantServicesSDK, Utility) {
    let client = BluefinTecsMerchantServicesSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinTecsMerchantServicesSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinTecsMerchantServicesError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinTecsMerchantServicesSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinTecsMerchantServicesError)?.code, "boom")
  }
