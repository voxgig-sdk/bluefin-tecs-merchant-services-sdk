...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: BluefinTecsMerchantServicesSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: BluefinTecsMerchantServicesSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> BluefinTecsMerchantServicesSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: BluefinTecsMerchantServicesSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.client = { options in
      ...
      return sdkProvider(BluefinTecsMerchantServicesSDK.testSDK(nil, opts), true)
    }

    // Client options may reference the runner store.
    provider.inject = { options, store in
      let o = tovalue(options)
      _ = BluefinTecsMerchantServicesSdk.inject(o, tovalue(store))
      return tojson(o)
    }
    ...
    provider.errify = { err in
      if let sdkerr = err as? BluefinTecsMerchantServicesError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("BluefinTecsMerchantServicesError")),
  ...
  final class Run {
    ...
    let client: BluefinTecsMerchantServicesSDK

    private let pack: Omni.RunPack

    fileprivate init(_ pack: Omni.RunPack, _ client: BluefinTecsMerchantServicesSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: BluefinTecsMerchantServicesSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: BluefinTecsMerchantServicesSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: BluefinTecsMerchantServicesSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: BluefinTecsMerchantServicesSDK, _ utility: Utility)
    -> Context
  {
