# BluefinTecsMerchantServices Swift — Agent Guide

The Swift client for the BluefinTecsMerchantServices API. This directory is **generated** — do not edit it by hand; change the model/template/component in `.sdk/` and regenerate. See the [project guide](../AGENTS.md) for the full workflow and the aontu model language.
...
npm run generate     # refreshes this swift/ directory
```

Then build and test the generated output:

```bash
# in this target directory (swift/):
make build
make test
```
...
| --- | --- | --- |
| **audit** — Structured audit trail of operations | `feature/audit_feature.swift` | `PreDone`, `PreUnexpected` |
| **clienttrack** — Client identity and per-request correlation headers | `feature/clienttrack_feature.swift` | `PostConstruct`, `PreRequest` |
| **idempotency** — Idempotency keys for safe retries of mutating operations | `feature/idempotency_feature.swift` | `PreRequest` |
| **log** — Structured request and response logging | `feature/log_feature.swift` | `GetData`, `GetMatch`, `PostConstruct`, `PostConstructEntity`, `PrePoint`, `PreRequest`, `PreResponse`, `PreResult`, `PreSpec`, `SetData`, `SetMatch` |
