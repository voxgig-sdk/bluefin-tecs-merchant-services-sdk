// payment_manual entity test - basic flow (generated from the API model).
...
public class PaymentManualEntityTest
{
    ...
    public void Basic()
    {
        var setup = PaymentManualBasicSetup(null);
        ...
        foreach (var _op in new[] { "create" })
        {
            var (_shouldSkip, _) = TestRunner.IsControlSkipped(
                "entityOp", "payment_manual." + _op, _mode);
            if (_shouldSkip)
            {
        ...
        // CREATE
        var paymentManualRef01Ent = client.PaymentManual();
        var paymentManualRef01Data = Helpers.ToMapAny(StructUtils.GetProp(
            StructUtils.GetPath(setup.Data, StructUtils.Jt("new", "payment_manual")),
    ...
    private static EntityTestSetup PaymentManualBasicSetup(
        Dictionary<string, object?>? extra)
    {
        TestRunner.LoadEnvLocal();

        var entityDataFile = Path.Combine(TestRunner.TestDir(),
            "..", "..", ".sdk", "test", "entity", "payment_manual",
            "PaymentManualTestData.json");
        ...
        var entityData = StructRunner.ConvertElement(entityDataEl)
            ...
            ?? throw new InvalidOperationException(
                "failed to parse payment_manual test data");
        ...
        var idmap = StructUtils.Transform(
            new List<object?> { "payment_manual01", "payment_manual02", "payment_manual03" },
