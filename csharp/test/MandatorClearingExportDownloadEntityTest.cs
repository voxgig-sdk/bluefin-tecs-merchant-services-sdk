// mandator_clearing_export_download entity test - basic flow (generated from the API model).
...
public class MandatorClearingExportDownloadEntityTest
{
    ...
    public void Instance()
    {
        ...
        var ent = testsdk.MandatorClearingExportDownload();
        Assert.NotNull(ent);
    }
    ...
    public void Basic()
    {
        var setup = MandatorClearingExportDownloadBasicSetup(null);
        ...
        foreach (var _op in new[] { "create", "load" })
        {
            var (_shouldSkip, _) = TestRunner.IsControlSkipped(
                "entityOp", "mandator_clearing_export_download." + _op, _mode);
            if (_shouldSkip)
            {
        ...
        // and 4xx; set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID JSON to run live.
        ...
        // CREATE
        var mandatorClearingExportDownloadRef01Ent = client.MandatorClearingExportDownload();
        var mandatorClearingExportDownloadRef01Data = Helpers.ToMapAny(StructUtils.GetProp(
            StructUtils.GetPath(setup.Data, StructUtils.Jt("new", "mandator_clearing_export_download")),
            "mandator_clearing_export_download_ref01"));
    ...
    private static EntityTestSetup MandatorClearingExportDownloadBasicSetup(
        Dictionary<string, object?>? extra)
    {
        TestRunner.LoadEnvLocal();

        var entityDataFile = Path.Combine(TestRunner.TestDir(),
            "..", "..", ".sdk", "test", "entity", "mandator_clearing_export_download",
            "MandatorClearingExportDownloadTestData.json");
        ...
        var entityData = StructRunner.ConvertElement(entityDataEl)
            ...
            ?? throw new InvalidOperationException(
                "failed to parse mandator_clearing_export_download test data");
        ...
        var idmap = StructUtils.Transform(
            new List<object?> { "mandator_clearing_export_download01", "mandator_clearing_export_download02", "mandator_clearing_export_download03" },
        ...
        var idmapResolved = Helpers.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID"])
