// version direct API tests (generated from the API model).
...
public class VersionDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = VersionDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-version", _mode);
        if (_shouldSkip)
        {
    ...
    private class VersionDirectSetupResult
    {
    ...
    private static VersionDirectSetupResult VersionDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_VERSION_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var liveClient = new BluefinTecsMerchantServicesSDK(liveOpts);

            var idmap = new Dictionary<string, object?>();
            var entidRaw = env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_VERSION_ENTID"];
            ...
            return new VersionDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
