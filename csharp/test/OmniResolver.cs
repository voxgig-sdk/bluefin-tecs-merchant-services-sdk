...
// `Util` alone would resolve to the SDK's BluefinTecsMerchantServicesSdk.Util namespace,
...
public static class OmniResolver
{
        ...
        var provider = new Provider
        {
            ...
            Client = options =>
            {
                var opts = FixNums(options) as Dictionary<string, object?>
                    ?? new Dictionary<string, object?>();
                var sub = SdkProvider(BluefinTecsMerchantServicesSDK.TestSDK(null, opts));
                MarkDefProvider(sub);
                return sub;
            },
            ...
            Errify = err =>
            {
                if (err is BluefinTecsMerchantServicesError sdkerr)
                {
                    var outmap = new Dictionary<string, object?>
                    {
    ...
    /// </summary>
    public static Context OmniCtx(object? arg, BluefinTecsMerchantServicesSDK client, Utility utility)
    {
        ...
        if (ctxmap.TryGetValue("client", out var p) && p is Provider prov &&
            DefProviderClient(prov) is BluefinTecsMerchantServicesSDK live)
        {
            client = live;
