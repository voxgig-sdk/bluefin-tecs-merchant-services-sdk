# BluefinTecsMerchantServices SDK response
...
package BluefinTecsMerchantServicesResponse;
...
sub new {
  ...
  my $jf = BluefinTecsMerchantServicesHelpers::gp($resmap, 'json');
  my $json_func = (ref $jf eq 'CODE') ? $jf : undef;

  return bless {
    status      => $status,
    status_text => $status_text,
    headers     => BluefinTecsMerchantServicesHelpers::gp($resmap, 'headers'),
    json_func   => $json_func,
