# BluefinTecsMerchantServices SDK utility: make_point
...
sub _parts_len {
  my ($point) = @_;
  my $parts = BluefinTecsMerchantServicesHelpers::gp($point, 'parts');
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinTecsMerchantServicesError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinTecsMerchantServicesHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    my $matched = 0;
    for my $p (@{ $op->{points} }) {
      my $select_def = BluefinTecsMerchantServicesHelpers::to_map(BluefinTecsMerchantServicesHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinTecsMerchantServicesHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinTecsMerchantServicesHelpers::gp($reqselector, "$ek");
            my $sv = BluefinTecsMerchantServicesHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinTecsMerchantServicesHelpers::gp($reqselector, '$action');
        my $select_action = BluefinTecsMerchantServicesHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinTecsMerchantServicesHelpers::eqv($req_action, $select_action);
      }
    ...
    unless ($matched) {
      ...
      my $unmatched_action = $reqselector
        ? BluefinTecsMerchantServicesHelpers::gp($reqselector, '$action') : undef;
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinTecsMerchantServicesHelpers::to_map(BluefinTecsMerchantServicesHelpers::gp($point, 'select'));
        my $point_action = BluefinTecsMerchantServicesHelpers::gp($point_select, '$action');
        unless (BluefinTecsMerchantServicesHelpers::eqv($req_action, $point_action)) {
          return (undef, $ctx->make_error('point_action_invalid',
            "Operation \"$op->{name}\" action \""
