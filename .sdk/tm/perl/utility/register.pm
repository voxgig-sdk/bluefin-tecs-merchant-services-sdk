...
$BluefinTecsMerchantServicesUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinTecsMerchantServicesUtilities::REGISTRY) {
    $u->{$k} = $BluefinTecsMerchantServicesUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinTecsMerchantServicesHelpers::struct_facade();
};
