# BluefinTecsMerchantServices SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinTecsMerchantServicesHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinTecsMerchantServicesHelpers::gp($ext, 'code');
  $raw = BluefinTecsMerchantServicesHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinTecsMerchantServicesHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinTecsMerchantServicesHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinTecsMerchantServicesHelpers::gp($spec, 'name');
    my $from = BluefinTecsMerchantServicesHelpers::gp($spec, 'from');
    ...
    my $val = BluefinTecsMerchantServicesHelpers::gp($reqsrc, $from);
    $val = BluefinTecsMerchantServicesHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinTecsMerchantServicesHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinTecsMerchantServicesHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinTecsMerchantServicesHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinTecsMerchantServicesHelpers::gp($first, 'message');
