# BluefinTecsMerchantServices SDK paging feature
#
...
package BluefinTecsMerchantServicesPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinTecsMerchantServicesHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (BluefinTecsMerchantServicesHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = BluefinTecsMerchantServicesHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = BluefinTecsMerchantServicesHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = BluefinTecsMerchantServicesHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = BluefinTecsMerchantServicesHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = BluefinTecsMerchantServicesHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = BluefinTecsMerchantServicesHelpers::gpath($conn, $morepath);
      if (BluefinTecsMerchantServicesHelpers::is_true($more) || BluefinTecsMerchantServicesHelpers::is_false($more)) {
        $paging->{hasMore} = BluefinTecsMerchantServicesHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinTecsMerchantServicesHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinTecsMerchantServicesHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinTecsMerchantServicesHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinTecsMerchantServicesHelpers::is_true($bhasmore) || BluefinTecsMerchantServicesHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinTecsMerchantServicesHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
