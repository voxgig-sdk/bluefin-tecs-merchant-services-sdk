#!perl
# BluefinTecsMerchantServices SDK entity stream() test
#
...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinTecsMerchantServicesConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinTecsMerchantServicesHelpers::to_map(
    BluefinTecsMerchantServicesHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinTecsMerchantServicesHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinTecsMerchantServicesHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinTecsMerchantServicesHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinTecsMerchantServicesSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinTecsMerchantServicesSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
