...
sub smoke_pack {
  my $runner = BluefinTecsMerchantServicesOmni::make_runner($SPEC, BluefinTecsMerchantServicesSDK->test(undef, undef));
  return $runner->('smoke');
}
...
# A wrong result fails, with an OmniError naming the mismatch.
{
  ...
  my $err = $@;
  ok(BluefinTecsMerchantServicesOmni::is_omni_error($err), 'the failure is an OmniError');
...
# must fail.
{
  ...
  my $err = $@;
  ok(BluefinTecsMerchantServicesOmni::is_omni_error($err), 'the missing-error failure is an OmniError');
