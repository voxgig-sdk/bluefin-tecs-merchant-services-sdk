...
{
  my $sdk = BluefinTecsMerchantServicesSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinTecsMerchantServicesSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinTecsMerchantServicesSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinTecsMerchantServicesSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinTecsMerchantServicesSDK->test(undef, undef);
