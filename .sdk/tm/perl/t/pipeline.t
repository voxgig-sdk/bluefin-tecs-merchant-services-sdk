...
sub resp {
  ...
  return BluefinTecsMerchantServicesResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinTecsMerchantServicesSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinTecsMerchantServicesError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinTecsMerchantServicesResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinTecsMerchantServicesError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinTecsMerchantServicesError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinTecsMerchantServicesSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinTecsMerchantServicesBaseFeature->new;
  my $b = BluefinTecsMerchantServicesBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinTecsMerchantServicesBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinTecsMerchantServicesHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinTecsMerchantServicesHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinTecsMerchantServicesContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinTecsMerchantServicesSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsMerchantServicesResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinTecsMerchantServicesResponse->new({
