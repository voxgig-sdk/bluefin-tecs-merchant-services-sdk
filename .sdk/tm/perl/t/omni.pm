...
#    `contextify` answers a TIED plain hash (BluefinTecsMerchantServicesOmniViewTie) whose
...
package BluefinTecsMerchantServicesOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinTecsMerchantServicesOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinTecsMerchantServicesError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinTecsMerchantServicesResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsMerchantServicesOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinTecsMerchantServicesOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinTecsMerchantServicesOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinTecsMerchantServicesOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinTecsMerchantServicesOmniProviderTie;
...
package BluefinTecsMerchantServicesOmniProvider;
...
# The resolver proper.
# --------------------------------------------------------------------------
package BluefinTecsMerchantServicesOmni;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinTecsMerchantServicesOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsMerchantServicesOmniViewTie') ? 1 : 0;
}
...
sub enrich {
  ...
  if (Voxgig::Struct::ismap($ctxmap->{spec}) && !Scalar::Util::blessed($ctxmap->{spec})) {
    $ctx->{spec} = BluefinTecsMerchantServicesSpec->new($ctxmap->{spec});
  }

  my $res_map = $ctxmap->{result};
  if (Voxgig::Struct::ismap($res_map) && !Scalar::Util::blessed($res_map)) {
    $ctx->{result} = BluefinTecsMerchantServicesResult->new($res_map);
    if (Voxgig::Struct::ismap($res_map->{err})
      && defined $res_map->{err}{message} && !ref $res_map->{err}{message}) {
      $ctx->{result}{err} = BluefinTecsMerchantServicesError->new('', $res_map->{err}{message});
    }
  }

  my $resp_map = $ctxmap->{response};
  if (Voxgig::Struct::ismap($resp_map) && !Scalar::Util::blessed($resp_map)) {
    $ctx->{response} = BluefinTecsMerchantServicesResponse->new($resp_map);
    if (BluefinTecsMerchantServicesHelpers::rb_truthy($resp_map->{body})) {
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinTecsMerchantServicesHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinTecsMerchantServicesOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinTecsMerchantServicesOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinTecsMerchantServicesContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinTecsMerchantServicesOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinTecsMerchantServicesOmniProvider';
}
