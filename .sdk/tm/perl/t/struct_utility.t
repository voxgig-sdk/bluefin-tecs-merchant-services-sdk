...
my $runner = BluefinTecsMerchantServicesOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinTecsMerchantServicesOmni::NULLMARK();
...
{
  my $td = BluefinTecsMerchantServicesOmni::tostruct(
...
{
  my $basic = BluefinTecsMerchantServicesOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinTecsMerchantServicesOmni::null_modifier }) });
