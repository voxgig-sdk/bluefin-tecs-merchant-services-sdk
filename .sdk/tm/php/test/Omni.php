...
//    only - a typed BluefinTecsMerchantServicesContext there would make every ctx
...
final class BluefinTecsMerchantServicesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BluefinTecsMerchantServicesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BluefinTecsMerchantServicesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BluefinTecsMerchantServicesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BluefinTecsMerchantServicesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BluefinTecsMerchantServicesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BluefinTecsMerchantServicesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BluefinTecsMerchantServicesError(is_string($code) ? $code : '', $msg);
    }
