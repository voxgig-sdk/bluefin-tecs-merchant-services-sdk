// BluefinTecsMerchantServices SDK — drives the primary utility functions against the shared
...
static res::Run& primaryRun() {
  static res::Run run =
      res::makeRunner(TEST_JSON_FILE, BluefinTecsMerchantServicesSDK::testSDK())
          .runner("primary", Value::undef());
  return run;
}

static std::shared_ptr<BluefinTecsMerchantServicesSDK> client() { return BluefinTecsMerchantServicesSDK::testSDK(); }
...
static CtxPtr makeTestCtx(std::shared_ptr<BluefinTecsMerchantServicesSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<BluefinTecsMerchantServicesSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<BluefinTecsMerchantServicesSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<BluefinTecsMerchantServicesSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = BluefinTecsMerchantServicesSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = BluefinTecsMerchantServicesSDK::testSDK(Value::undef(), setupOpts);
