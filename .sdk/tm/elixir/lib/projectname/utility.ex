# BluefinTecsMerchantServices SDK utility
#
...
defmodule BluefinTecsMerchantServices.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantServices.Helpers, as: H
  alias BluefinTecsMerchantServices.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinTecsMerchantServicesSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinTecsMerchantServices.Config.shared_config) and merge uses its nested nodes as
      ...
      resolved =
        Regex.replace(~r/\{([A-Za-z0-9_]+)\}/, base, fn _match, name ->
          ...
          cond do
            ...
            true ->
              raise BluefinTecsMerchantServices.Error,
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinTecsMerchantServices.Error{}, err) -> err.msg
    ...
    sdk_err = %BluefinTecsMerchantServices.Error{
      code: "",
      msg: msg,
      sdk: "BluefinTecsMerchantServices",
      ctx: ctx,
    ...
    sdk_err = if match?(%BluefinTecsMerchantServices.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinTecsMerchantServices.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinTecsMerchantServices.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
