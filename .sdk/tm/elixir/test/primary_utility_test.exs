...
defmodule BluefinTecsMerchantServices.PrimaryUtilityTest do
  ...
  alias BluefinTecsMerchantServices.{Context, Utility, Spec, Result}

  defp client, do: BluefinTecsMerchantServices.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinTecsMerchantServices.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinTecsMerchantServices.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "prepare_auth omits the header when no apikey" do
    c = BluefinTecsMerchantServices.new()
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinTecsMerchantServices.test()
    opts = BluefinTecsMerchantServices.options_map(c)
