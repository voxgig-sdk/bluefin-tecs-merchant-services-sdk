# Vendored struct corpus runner for the BluefinTecsMerchantServices SDK.
#
...
defmodule BluefinTecsMerchantServices.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinTecsMerchantServices.test(nil, S.clone(setup)), else: BluefinTecsMerchantServices.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinTecsMerchantServices.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinTecsMerchantServices.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinTecsMerchantServices.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinTecsMerchantServices.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
