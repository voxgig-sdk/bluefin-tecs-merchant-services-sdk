...
test "digital_services_api_load_smoke" {
    const fixture = h.jo(&.{.{ "digital_services_api", h.jo(&.{.{ "t01", h.jo(&.{.{ "id", h.vstr("t01") }}) }}) }});
    const testsdk = sdk.test_sdk(h.jo(&.{.{ "entity", fixture }}), vnull());
    const e = testsdk.digital_services_api(vnull());
    ...
    switch (res) {
        ...
        .err => |er| {
            std.debug.print("digital_services_api load failed: {s}\n", .{er.msg});
            try std.testing.expect(false);
        },
    }
}

test "digital_services_api_direct_smoke" {
    ...
    const result = testsdk.direct(h.jo(&.{
        .{ "path", h.vstr("/digital_services_api/{id}") },
        .{ "method", h.vstr("GET") },
...
test "digital_services_api_prepare_smoke" {
...
test "mandator_clearing_export_download_load_smoke" {
    const fixture = h.jo(&.{.{ "mandator_clearing_export_download", h.jo(&.{.{ "t01", h.jo(&.{.{ "id", h.vstr("t01") }}) }}) }});
    const testsdk = sdk.test_sdk(h.jo(&.{.{ "entity", fixture }}), vnull());
    const e = testsdk.mandator_clearing_export_download(vnull());
    const res = e.load(h.jo(&.{.{ "id", h.vstr("t01") }}), vnull());
    switch (res) {
        .ok => |data| {
            try std.testing.expect(std.mem.eql(u8, h.get_str(data, "id") orelse "", "t01"));
        ...
        .err => |er| {
            std.debug.print("mandator_clearing_export_download load failed: {s}\n", .{er.msg});
            try std.testing.expect(false);
        },
    }
}

test "mandator_clearing_export_download_direct_smoke" {
    // direct() drives prepare -> transport and always returns a result map
    // carrying an `ok` flag (never an error union), even on a non-2xx or a
    // prepare failure.
    const testsdk = sdk.test_sdk(vnull(), vnull());
    const result = testsdk.direct(h.jo(&.{
        .{ "path", h.vstr("/mandator_clearing_export_download/{id}") },
        .{ "method", h.vstr("GET") },
        .{ "params", h.jo(&.{.{ "id", h.vstr("direct01") }}) },
    }));
    try std.testing.expect(result == .object);
...
test "mandator_clearing_export_download_prepare_smoke" {
    // prepare() returns the fetch definition (an error union). The generated
    // fetchdef always carries a url + method.
    const testsdk = sdk.test_sdk(vnull(), vnull());
    const fetchdef = testsdk.prepare(h.jo(&.{
