// MandatorClearingExportDownload entity client (generated — mirrors the go/rust Entity fragment).
...
pub const MandatorClearingExportDownloadEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *MandatorClearingExportDownloadEntity {
        ...
        const e = h.A().create(MandatorClearingExportDownloadEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *MandatorClearingExportDownloadEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *MandatorClearingExportDownloadEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *MandatorClearingExportDownloadEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *MandatorClearingExportDownloadEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return MandatorClearingExportDownloadEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *MandatorClearingExportDownloadEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *MandatorClearingExportDownloadEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *MandatorClearingExportDownloadEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
