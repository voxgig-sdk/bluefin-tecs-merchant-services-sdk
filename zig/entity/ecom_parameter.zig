// EcomParameter entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EcomParameterEntity = struct {
    name: []const u8 = "ecom_parameter",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *EcomParameterEntity {
        ...
        const e = h.A().create(EcomParameterEntity) catch unreachable;
        e.* = .{
            .name = "ecom_parameter",
            .client = client,
    ...
    fn ent_ctx(self: *EcomParameterEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EcomParameterEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EcomParameterEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EcomParameterEntity, ctx: *Context, post_done: *const fn (*EcomParameterEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EcomParameterEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EcomParameterEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EcomParameterEntity, ctx: *Context, post_done: *const fn (*EcomParameterEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EcomParameterEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EcomParameterEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EcomParameterEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EcomParameterEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EcomParameterEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EcomParameterEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EcomParameterEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *EcomParameterEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EcomParameterEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *EcomParameterEntity, ctx: *Context) void {
    ...
    pub fn update(self: *EcomParameterEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EcomParameterEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
