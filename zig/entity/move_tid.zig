// MoveTid entity client (generated — mirrors the go/rust Entity fragment).
...
pub const MoveTidEntity = struct {
    name: []const u8 = "move_tid",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *MoveTidEntity {
        ...
        const e = h.A().create(MoveTidEntity) catch unreachable;
        e.* = .{
            .name = "move_tid",
            .client = client,
    ...
    fn ent_ctx(self: *MoveTidEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *MoveTidEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *MoveTidEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *MoveTidEntity, ctx: *Context, post_done: *const fn (*MoveTidEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *MoveTidEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *MoveTidEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *MoveTidEntity, ctx: *Context, post_done: *const fn (*MoveTidEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *MoveTidEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *MoveTidEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return MoveTidEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *MoveTidEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *MoveTidEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *MoveTidEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *MoveTidEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *MoveTidEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *MoveTidEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *MoveTidEntity, ctx: *Context) void {
    ...
    pub fn update(self: *MoveTidEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *MoveTidEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
