// DigitalServicesApi entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DigitalServicesApiEntity = struct {
    name: []const u8 = "digital_services_api",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *DigitalServicesApiEntity {
        ...
        const e = h.A().create(DigitalServicesApiEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *DigitalServicesApiEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DigitalServicesApiEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DigitalServicesApiEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DigitalServicesApiEntity, ctx: *Context, post_done: *const fn (*DigitalServicesApiEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DigitalServicesApiEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DigitalServicesApiEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DigitalServicesApiEntity, ctx: *Context, post_done: *const fn (*DigitalServicesApiEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DigitalServicesApiEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DigitalServicesApiEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DigitalServicesApiEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DigitalServicesApiEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DigitalServicesApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DigitalServicesApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DigitalServicesApiEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *DigitalServicesApiEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DigitalServicesApiEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *DigitalServicesApiEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *DigitalServicesApiEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *DigitalServicesApiEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DigitalServicesApiEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
