// TransactionsCountCardBrand entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TransactionsCountCardBrandEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *TransactionsCountCardBrandEntity {
        ...
        const e = h.A().create(TransactionsCountCardBrandEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *TransactionsCountCardBrandEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TransactionsCountCardBrandEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *TransactionsCountCardBrandEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TransactionsCountCardBrandEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TransactionsCountCardBrandEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TransactionsCountCardBrandEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TransactionsCountCardBrandEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TransactionsCountCardBrandEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
