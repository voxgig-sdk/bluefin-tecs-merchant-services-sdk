// EcDataEcom entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EcDataEcomEntity = struct {
    name: []const u8 = "ec_data_ecom",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *EcDataEcomEntity {
        ...
        const e = h.A().create(EcDataEcomEntity) catch unreachable;
        e.* = .{
            .name = "ec_data_ecom",
            .client = client,
    ...
    fn ent_ctx(self: *EcDataEcomEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EcDataEcomEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EcDataEcomEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EcDataEcomEntity, ctx: *Context, post_done: *const fn (*EcDataEcomEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EcDataEcomEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EcDataEcomEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EcDataEcomEntity, ctx: *Context, post_done: *const fn (*EcDataEcomEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EcDataEcomEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EcDataEcomEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EcDataEcomEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EcDataEcomEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EcDataEcomEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EcDataEcomEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EcDataEcomEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *EcDataEcomEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EcDataEcomEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *EcDataEcomEntity, ctx: *Context) void {
    ...
    pub fn update(self: *EcDataEcomEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EcDataEcomEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
