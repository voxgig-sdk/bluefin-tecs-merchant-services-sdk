// Version entity client (generated — mirrors the go/rust Entity fragment).
...
pub const VersionEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *VersionEntity {
        ...
        const e = h.A().create(VersionEntity) catch unreachable;
        e.* = .{
            .name = "version",
            .client = client,
    ...
    fn ent_ctx(self: *VersionEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *VersionEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *VersionEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *VersionEntity, ctx: *Context, post_done: *const fn (*VersionEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *VersionEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *VersionEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *VersionEntity, ctx: *Context, post_done: *const fn (*VersionEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *VersionEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *VersionEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return VersionEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *VersionEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *VersionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *VersionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *VersionEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *VersionEntity, ctx: *Context) void {
    ...
    pub fn list(self: *VersionEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *VersionEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *VersionEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *VersionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
