// RegisterTerminal entity client (generated — mirrors the go/rust Entity fragment).
...
pub const RegisterTerminalEntity = struct {
    name: []const u8 = "register_terminal",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *RegisterTerminalEntity {
        ...
        const e = h.A().create(RegisterTerminalEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *RegisterTerminalEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *RegisterTerminalEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *RegisterTerminalEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *RegisterTerminalEntity, ctx: *Context, post_done: *const fn (*RegisterTerminalEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *RegisterTerminalEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *RegisterTerminalEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *RegisterTerminalEntity, ctx: *Context, post_done: *const fn (*RegisterTerminalEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *RegisterTerminalEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *RegisterTerminalEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return RegisterTerminalEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *RegisterTerminalEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *RegisterTerminalEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *RegisterTerminalEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *RegisterTerminalEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *RegisterTerminalEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *RegisterTerminalEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *RegisterTerminalEntity, ctx: *Context) void {
    ...
    pub fn update(self: *RegisterTerminalEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *RegisterTerminalEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
