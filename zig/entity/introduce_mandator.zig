// IntroduceMandator entity client (generated — mirrors the go/rust Entity fragment).
...
pub const IntroduceMandatorEntity = struct {
    name: []const u8 = "introduce_mandator",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *IntroduceMandatorEntity {
        ...
        const e = h.A().create(IntroduceMandatorEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *IntroduceMandatorEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *IntroduceMandatorEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *IntroduceMandatorEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *IntroduceMandatorEntity, ctx: *Context, post_done: *const fn (*IntroduceMandatorEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *IntroduceMandatorEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *IntroduceMandatorEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *IntroduceMandatorEntity, ctx: *Context, post_done: *const fn (*IntroduceMandatorEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *IntroduceMandatorEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *IntroduceMandatorEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return IntroduceMandatorEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *IntroduceMandatorEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *IntroduceMandatorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *IntroduceMandatorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *IntroduceMandatorEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *IntroduceMandatorEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *IntroduceMandatorEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *IntroduceMandatorEntity, ctx: *Context) void {
    ...
    pub fn update(self: *IntroduceMandatorEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *IntroduceMandatorEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
