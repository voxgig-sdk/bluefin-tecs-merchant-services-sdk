// EnableAcquiring entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EnableAcquiringEntity = struct {
    name: []const u8 = "enable_acquiring",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *EnableAcquiringEntity {
        ...
        const e = h.A().create(EnableAcquiringEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *EnableAcquiringEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EnableAcquiringEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EnableAcquiringEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EnableAcquiringEntity, ctx: *Context, post_done: *const fn (*EnableAcquiringEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EnableAcquiringEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EnableAcquiringEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EnableAcquiringEntity, ctx: *Context, post_done: *const fn (*EnableAcquiringEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EnableAcquiringEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EnableAcquiringEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EnableAcquiringEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EnableAcquiringEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EnableAcquiringEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EnableAcquiringEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EnableAcquiringEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *EnableAcquiringEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EnableAcquiringEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *EnableAcquiringEntity, ctx: *Context) void {
    ...
    pub fn update(self: *EnableAcquiringEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EnableAcquiringEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
