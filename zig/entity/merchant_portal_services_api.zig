// MerchantPortalServicesApi entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *MerchantPortalServicesApiEntity,
...
pub const MerchantPortalServicesApiEntity = struct {
    name: []const u8 = "merchant_portal_services_api",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *MerchantPortalServicesApiEntity {
        ...
        const e = h.A().create(MerchantPortalServicesApiEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *MerchantPortalServicesApiEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *MerchantPortalServicesApiEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *MerchantPortalServicesApiEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *MerchantPortalServicesApiEntity, ctx: *Context, post_done: *const fn (*MerchantPortalServicesApiEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *MerchantPortalServicesApiEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *MerchantPortalServicesApiEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *MerchantPortalServicesApiEntity, ctx: *Context, post_done: *const fn (*MerchantPortalServicesApiEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *MerchantPortalServicesApiEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *MerchantPortalServicesApiEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return MerchantPortalServicesApiEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *MerchantPortalServicesApiEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *MerchantPortalServicesApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *MerchantPortalServicesApiEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *MerchantPortalServicesApiEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *MerchantPortalServicesApiEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *MerchantPortalServicesApiEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *MerchantPortalServicesApiEntity, ctx: *Context) void {
    ...
    pub fn update(self: *MerchantPortalServicesApiEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *MerchantPortalServicesApiEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
