// ListTerminal entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ListTerminalEntity = struct {
    name: []const u8 = "list_terminal",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *ListTerminalEntity {
        ...
        const e = h.A().create(ListTerminalEntity) catch unreachable;
        e.* = .{
            .name = "list_terminal",
            .client = client,
    ...
    fn ent_ctx(self: *ListTerminalEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ListTerminalEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ListTerminalEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ListTerminalEntity, ctx: *Context, post_done: *const fn (*ListTerminalEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ListTerminalEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ListTerminalEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ListTerminalEntity, ctx: *Context, post_done: *const fn (*ListTerminalEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ListTerminalEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ListTerminalEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ListTerminalEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ListTerminalEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ListTerminalEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ListTerminalEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ListTerminalEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *ListTerminalEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *ListTerminalEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *ListTerminalEntity, ctx: *Context) void {
    ...
    pub fn update(self: *ListTerminalEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ListTerminalEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
