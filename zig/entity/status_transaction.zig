// StatusTransaction entity client (generated — mirrors the go/rust Entity fragment).
...
pub const StatusTransactionEntity = struct {
    name: []const u8 = "status_transaction",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *StatusTransactionEntity {
        ...
        const e = h.A().create(StatusTransactionEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *StatusTransactionEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *StatusTransactionEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *StatusTransactionEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *StatusTransactionEntity, ctx: *Context, post_done: *const fn (*StatusTransactionEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *StatusTransactionEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *StatusTransactionEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *StatusTransactionEntity, ctx: *Context, post_done: *const fn (*StatusTransactionEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *StatusTransactionEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *StatusTransactionEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return StatusTransactionEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *StatusTransactionEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *StatusTransactionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *StatusTransactionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *StatusTransactionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *StatusTransactionEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *StatusTransactionEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *StatusTransactionEntity, ctx: *Context) void {
    ...
    pub fn update(self: *StatusTransactionEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *StatusTransactionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
