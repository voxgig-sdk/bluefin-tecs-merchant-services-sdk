// MandatorClearingExport entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *MandatorClearingExportEntity,
...
pub const MandatorClearingExportEntity = struct {
    name: []const u8 = "mandator_clearing_export",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *MandatorClearingExportEntity {
        ...
        const e = h.A().create(MandatorClearingExportEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *MandatorClearingExportEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *MandatorClearingExportEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *MandatorClearingExportEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *MandatorClearingExportEntity, ctx: *Context, post_done: *const fn (*MandatorClearingExportEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *MandatorClearingExportEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *MandatorClearingExportEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *MandatorClearingExportEntity, ctx: *Context, post_done: *const fn (*MandatorClearingExportEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *MandatorClearingExportEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *MandatorClearingExportEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return MandatorClearingExportEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *MandatorClearingExportEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *MandatorClearingExportEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *MandatorClearingExportEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *MandatorClearingExportEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *MandatorClearingExportEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *MandatorClearingExportEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *MandatorClearingExportEntity, ctx: *Context) void {
    ...
    pub fn update(self: *MandatorClearingExportEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *MandatorClearingExportEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
