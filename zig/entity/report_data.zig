// ReportData entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ReportDataEntity = struct {
    name: []const u8 = "report_data",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantServicesSDK, entopts_in: Value) *ReportDataEntity {
        ...
        const e = h.A().create(ReportDataEntity) catch unreachable;
        e.* = .{
            .name = "report_data",
            .client = client,
    ...
    fn ent_ctx(self: *ReportDataEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ReportDataEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ReportDataEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ReportDataEntity, ctx: *Context, post_done: *const fn (*ReportDataEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ReportDataEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ReportDataEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ReportDataEntity, ctx: *Context, post_done: *const fn (*ReportDataEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ReportDataEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ReportDataEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ReportDataEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ReportDataEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ReportDataEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ReportDataEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ReportDataEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *ReportDataEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *ReportDataEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *ReportDataEntity, ctx: *Context) void {
    ...
    pub fn update(self: *ReportDataEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ReportDataEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
