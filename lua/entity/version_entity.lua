...
local VersionEntity = {}
VersionEntity.__index = VersionEntity


function VersionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VersionEntity)
  self._name = "version"
  self._client = client
...
function VersionEntity:get_name()
  return self._name
end


function VersionEntity:make()
  local opts = {}
...
function VersionEntity:mark_deleted()
  self._deleted = true
end


function VersionEntity:deleted()
  return true == self._deleted
end


function VersionEntity:data_set(args)
  if args ~= nil then
...
function VersionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VersionEntity:match_set(args)
  if args ~= nil then
...
function VersionEntity:match_get()
...
function VersionEntity:stream(action, args, callopts)
...
---@param reqmatch VersionLoadMatch
---@param ctrl? table
---@return Version
---@return string? err
function VersionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function VersionEntity:_run_op(ctx, post_done)
  local utility = self._utility
