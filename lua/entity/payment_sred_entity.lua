...
local PaymentSredEntity = {}
PaymentSredEntity.__index = PaymentSredEntity


function PaymentSredEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentSredEntity)
  self._name = "payment_sred"
  self._client = client
...
function PaymentSredEntity:get_name()
  return self._name
end


function PaymentSredEntity:make()
  local opts = {}
...
function PaymentSredEntity:mark_deleted()
  self._deleted = true
end


function PaymentSredEntity:deleted()
  return true == self._deleted
end


function PaymentSredEntity:data_set(args)
  if args ~= nil then
...
function PaymentSredEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentSredEntity:match_set(args)
  if args ~= nil then
...
function PaymentSredEntity:match_get()
...
function PaymentSredEntity:stream(action, args, callopts)
...
---@param reqdata PaymentSredCreateData
---@param ctrl? table
---@return PaymentSred
---@return string? err
function PaymentSredEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PaymentSredEntity:_run_op(ctx, post_done)
  local utility = self._utility
