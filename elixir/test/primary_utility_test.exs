...
defmodule BluefinTecsMerchantServices.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantServices.Omni, as: O
  alias BluefinTecsMerchantServices.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinTecsMerchantServices.get_utility(sdk)
    rootctx = BluefinTecsMerchantServices.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinTecsMerchantServices.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinTecsMerchantServices.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinTecsMerchantServices.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinTecsMerchantServices.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinTecsMerchantServices.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinTecsMerchantServices.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinTecsMerchantServices.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinTecsMerchantServices.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinTecsMerchantServices.test(nil, setup_opts(spec, "prepareAuth"))
