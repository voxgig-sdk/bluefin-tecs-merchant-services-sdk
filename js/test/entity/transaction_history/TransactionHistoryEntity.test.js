...
describe('TransactionHistoryEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        transaction_history: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.TransactionHistory().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.TransactionHistory().stream('list')) {
