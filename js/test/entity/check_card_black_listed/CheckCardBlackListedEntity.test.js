...
describe('CheckCardBlackListedEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        check_card_black_listed: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.CheckCardBlackListed().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.CheckCardBlackListed().stream('list')) {
