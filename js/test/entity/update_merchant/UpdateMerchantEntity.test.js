...
describe('UpdateMerchantEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        update_merchant: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.UpdateMerchant().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.UpdateMerchant().stream('list')) {
