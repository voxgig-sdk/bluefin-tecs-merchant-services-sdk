...
describe('MandatorClearingExportDownloadEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        mandator_clearing_export_download: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.MandatorClearingExportDownload().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.MandatorClearingExportDownload().stream('list')) {
  ...
  test('basic', async () => {
    ...
    const mandator_clearing_export_download_ref01_data_dt0 = await mandator_clearing_export_download_ref01_ent.load(mandator_clearing_export_download_ref01_match_dt0)
