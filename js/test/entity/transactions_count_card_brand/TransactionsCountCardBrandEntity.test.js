...
describe('TransactionsCountCardBrandEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        transactions_count_card_brand: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.TransactionsCountCardBrand().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.TransactionsCountCardBrand().stream('list')) {
