...
import voxgig.bluefintecsmerchantservicessdk.core._
import voxgig.bluefintecsmerchantservicessdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: BluefinTecsMerchantServicesSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => BluefinTecsMerchantServicesSDK): BluefinTecsMerchantServicesSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): BluefinTecsMerchantServicesSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new BluefinTecsMerchantServicesSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): BluefinTecsMerchantServicesSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      BluefinTecsMerchantServicesSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: BluefinTecsMerchantServicesSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: BluefinTecsMerchantServicesSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: BluefinTecsMerchantServicesSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = BluefinTecsMerchantServicesSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = BluefinTecsMerchantServicesSDK.testSDK(null, null)
