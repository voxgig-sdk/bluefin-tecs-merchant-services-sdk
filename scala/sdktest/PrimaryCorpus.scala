...
import voxgig.bluefintecsmerchantservicessdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefintecsmerchantservicessdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinTecsMerchantServicesSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinTecsMerchantServicesSDK)(f: (Context, Seq[Value]) => Value): Unit =
  ...
  private def clientFor(name: String, shared: BluefinTecsMerchantServicesSDK): BluefinTecsMerchantServicesSDK =
    sectionSetup(name) match {
      case m @ VMap(_) => BluefinTecsMerchantServicesSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinTecsMerchantServicesSDK.testSDK()
