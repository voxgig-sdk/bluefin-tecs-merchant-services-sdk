// Generated direct-call tests for the digital_services_api entity (mirrors the java
...
object DigitalServicesApiDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-digital_services_api") {
      ...
      rep.eq("direct-load-digital_services_api.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-digital_services_api.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-digital_services_api.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-digital_services_api.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-digital_services_api.calls", 1, setup.calls.size())
    }
  }
}
