...
func TestRegisterTecsCompanyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		registerTecsCompanyRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "register_tecs_company"}), "register_tecs_company_ref01"))

		registerTecsCompanyRef01DataResult, err := registerTecsCompanyRef01Ent.Create(registerTecsCompanyRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		registerTecsCompanyRef01Data = core.ToMapAny(entityData(registerTecsCompanyRef01DataResult))
...
func register_tecs_companyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TECS_COMPANY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TECS_COMPANY_ENTID"])
