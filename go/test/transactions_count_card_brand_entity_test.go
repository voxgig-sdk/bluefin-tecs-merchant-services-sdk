...
func TestTransactionsCountCardBrandEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		transactionsCountCardBrandRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "transactions_count_card_brand"}), "transactions_count_card_brand_ref01"))
		...
		transactionsCountCardBrandRef01Data = core.ToMapAny(entityData(transactionsCountCardBrandRef01DataResult))
...
func transactions_count_card_brandBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTIONS_COUNT_CARD_BRAND_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTIONS_COUNT_CARD_BRAND_ENTID"])
