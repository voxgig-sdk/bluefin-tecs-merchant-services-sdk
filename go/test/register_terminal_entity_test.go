...
func TestRegisterTerminalEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		registerTerminalRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "register_terminal"}), "register_terminal_ref01"))

		registerTerminalRef01DataResult, err := registerTerminalRef01Ent.Create(registerTerminalRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		registerTerminalRef01Data = core.ToMapAny(entityData(registerTerminalRef01DataResult))
...
func register_terminalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TERMINAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TERMINAL_ENTID"])
