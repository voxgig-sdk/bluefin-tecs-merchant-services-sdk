...
func TestPaymentSredEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		paymentSredRef01Data = core.ToMapAny(paymentSredRef01DataResult)
...
func payment_sredBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID"])
