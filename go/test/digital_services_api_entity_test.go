...
func TestDigitalServicesApiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		digitalServicesApiRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "digital_services_api"}), "digital_services_api_ref01"))

		digitalServicesApiRef01DataResult, err := digitalServicesApiRef01Ent.Create(digitalServicesApiRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		digitalServicesApiRef01Data = core.ToMapAny(entityData(digitalServicesApiRef01DataResult))
...
func digital_services_apiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_DIGITAL_SERVICES_API_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_DIGITAL_SERVICES_API_ENTID"])
