...
func TestEcDataEcomEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		ecDataEcomRef01Data = core.ToMapAny(ecDataEcomRef01DataResult)
...
func ec_data_ecomBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID"])
