...
func TestMerchantPortalServicesApiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		merchantPortalServicesApiRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "merchant_portal_services_api"}), "merchant_portal_services_api_ref01"))
		...
		merchantPortalServicesApiRef01Data = core.ToMapAny(entityData(merchantPortalServicesApiRef01DataResult))
...
func merchant_portal_services_apiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID"])
