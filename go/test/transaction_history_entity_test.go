...
func TestTransactionHistoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		transactionHistoryRef01Data = core.ToMapAny(transactionHistoryRef01DataResult)
...
func transaction_historyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID"])
