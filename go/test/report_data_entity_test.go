...
func TestReportDataEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		reportDataRef01Data = core.ToMapAny(entityData(reportDataRef01DataResult))
...
func report_dataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID")
