...
func TestGetTemplateXmlEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.GetTemplateXml(nil)
		if ent == nil {
			t.Fatal("expected non-nil GetTemplateXmlEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := get_template_xmlBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "get_template_xml." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		getTemplateXmlRef01Data = core.ToMapAny(getTemplateXmlRef01DataResult)
		if getTemplateXmlRef01Data == nil {
...
func get_template_xmlBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "get_template_xml", "GetTemplateXmlTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read get_template_xml test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse get_template_xml test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID"])
