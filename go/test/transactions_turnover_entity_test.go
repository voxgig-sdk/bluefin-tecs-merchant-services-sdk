...
func TestTransactionsTurnoverEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.TransactionsTurnover(nil)
		if ent == nil {
			t.Fatal("expected non-nil TransactionsTurnoverEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := transactions_turnoverBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "transactions_turnover." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		// CREATE
		transactionsTurnoverRef01Ent := client.TransactionsTurnover(nil)
		transactionsTurnoverRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "transactions_turnover"}, setup.data), "transactions_turnover_ref01"))

		transactionsTurnoverRef01DataResult, err := transactionsTurnoverRef01Ent.Create(transactionsTurnoverRef01Data, nil)
		if err != nil {
...
func transactions_turnoverBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "transactions_turnover", "TransactionsTurnoverTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read transactions_turnover test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse transactions_turnover test data: " + err.Error())
	}
	...
	idmap := vs.Transform(
		[]any{"transactions_turnover01", "transactions_turnover02", "transactions_turnover03"},
		map[string]any{
