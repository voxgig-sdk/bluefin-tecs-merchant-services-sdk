...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-tecs-merchant-services-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinTecsMerchantServicesError); is {
				out := map[string]any{
					"name":    "BluefinTecsMerchantServicesError",
...
func omniCtx(arg any, client *sdk.BluefinTecsMerchantServicesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinTecsMerchantServicesSDK); is {
			client = live
