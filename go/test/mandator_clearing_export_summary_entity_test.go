...
func TestMandatorClearingExportSummaryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		mandatorClearingExportSummaryRef01Data = core.ToMapAny(entityData(mandatorClearingExportSummaryRef01DataResult))
...
func mandator_clearing_export_summaryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID")
