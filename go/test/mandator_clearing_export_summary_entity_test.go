...
func TestMandatorClearingExportSummaryEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.MandatorClearingExportSummary(nil)
		if ent == nil {
			t.Fatal("expected non-nil MandatorClearingExportSummaryEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := mandator_clearing_export_summaryBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "mandator_clearing_export_summary." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		mandatorClearingExportSummaryRef01Data = core.ToMapAny(mandatorClearingExportSummaryRef01DataResult)
		if mandatorClearingExportSummaryRef01Data == nil {
...
func mandator_clearing_export_summaryBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "mandator_clearing_export_summary", "MandatorClearingExportSummaryTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read mandator_clearing_export_summary test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse mandator_clearing_export_summary test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID"])
