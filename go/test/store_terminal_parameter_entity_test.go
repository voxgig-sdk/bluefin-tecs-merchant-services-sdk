...
func TestStoreTerminalParameterEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		storeTerminalParameterRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "store_terminal_parameter"}), "store_terminal_parameter_ref01"))

		storeTerminalParameterRef01DataResult, err := storeTerminalParameterRef01Ent.Create(storeTerminalParameterRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		storeTerminalParameterRef01Data = core.ToMapAny(entityData(storeTerminalParameterRef01DataResult))
...
func store_terminal_parameterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID"])
