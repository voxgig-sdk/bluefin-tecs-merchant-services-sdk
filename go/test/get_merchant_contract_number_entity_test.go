...
func TestGetMerchantContractNumberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_GET_MERCHANT_CONTRACT_NUMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		getMerchantContractNumberRef01Data = core.ToMapAny(getMerchantContractNumberRef01DataResult)
...
func get_merchant_contract_numberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINTECSMERCHANTSERVICES_TEST_GET_MERCHANT_CONTRACT_NUMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_GET_MERCHANT_CONTRACT_NUMBER_ENTID"])
