...
class DigitalServicesApiEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DigitalServicesApiEntity(self._client, opts)
    ...
    def data_get(self) -> DigitalServicesApi:
    ...
    def match_get(self) -> DigitalServicesApi:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DigitalServicesApi:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DigitalServicesApi().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DigitalServicesApiCreateData, ctrl=None) -> DigitalServicesApi:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
