...
from bluefintecsmerchantservices_sdk.bluefintecsmerchantservices_types import (
...
class MandatorClearingExportDownloadEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MandatorClearingExportDownloadEntity(self._client, opts)
    ...
    def data_get(self) -> MandatorClearingExportDownload:
    ...
    def match_get(self) -> MandatorClearingExportDownload:
    ...
    def load(self, reqmatch=None, ctrl=None) -> MandatorClearingExportDownload:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.MandatorClearingExportDownload().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: MandatorClearingExportDownloadCreateData, ctrl=None) -> MandatorClearingExportDownload:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
