...
class TestPreAuthTransactionCompletionEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.PreAuthTransactionCompletion(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _pre_auth_transaction_completion_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "pre_auth_transaction_completion." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINTECSMERCHANTSERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        pre_auth_transaction_completion_ref01_ent = client.PreAuthTransactionCompletion(None)
        ...
        pre_auth_transaction_completion_ref01_data = helpers.to_map(pre_auth_transaction_completion_ref01_ent.create(pre_auth_transaction_completion_ref01_data, None))
        assert pre_auth_transaction_completion_ref01_data is not None



def _pre_auth_transaction_completion_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/pre_auth_transaction_completion/PreAuthTransactionCompletionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFINTECSMERCHANTSERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINTECSMERCHANTSERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID"))
