    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_KEEP_ALIVE_ENTID JSON to run live")
        client = setup["client"]
        ...
        keep_alive_ref01_data = helpers.to_map(runner.entity_data(keep_alive_ref01_ent.create(keep_alive_ref01_data, None)))
...
def _keep_alive_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_KEEP_ALIVE_ENTID"))
