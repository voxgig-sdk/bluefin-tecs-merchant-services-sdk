    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TERMINAL_ENTID JSON to run live")
        client = setup["client"]
        ...
        register_terminal_ref01_data = helpers.to_map(runner.entity_data(register_terminal_ref01_ent.create(register_terminal_ref01_data, None)))
...
def _register_terminal_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TERMINAL_ENTID"))
