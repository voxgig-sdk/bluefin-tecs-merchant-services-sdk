...
class TestMoveTidEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.MoveTid(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _move_tid_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "move_tid." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MOVE_TID_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        move_tid_ref01_ent = client.MoveTid(None)
        move_tid_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.move_tid"), "move_tid_ref01"))

        move_tid_ref01_data = helpers.to_map(runner.entity_data(move_tid_ref01_ent.create(move_tid_ref01_data, None)))
        assert move_tid_ref01_data is not None



def _move_tid_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/move_tid/MoveTidTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MOVE_TID_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MOVE_TID_ENTID"))
