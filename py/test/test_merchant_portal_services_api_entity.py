...
class TestMerchantPortalServicesApiEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.MerchantPortalServicesApi(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _merchant_portal_services_api_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "merchant_portal_services_api." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        merchant_portal_services_api_ref01_ent = client.MerchantPortalServicesApi(None)
        ...
        merchant_portal_services_api_ref01_data = helpers.to_map(runner.entity_data(merchant_portal_services_api_ref01_ent.create(merchant_portal_services_api_ref01_data, None)))
        assert merchant_portal_services_api_ref01_data is not None



def _merchant_portal_services_api_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/merchant_portal_services_api/MerchantPortalServicesApiTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID"))
