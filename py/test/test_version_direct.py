...
class TestVersionDirect:

    def test_should_direct_load_version(self):
        setup = _version_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-version", "live" if setup["live"] else "unit")
        if _skip:
...
def _version_direct_setup(mockres):
