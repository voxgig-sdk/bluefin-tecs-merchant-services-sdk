# BluefinTecsMerchantServices SDK test runner
...
class BluefinTecsMerchantServicesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinTecsMerchantServicesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinTecsMerchantServicesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinTecsMerchantServicesTestRunner.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_LIVE")
        override = BluefinTecsMerchantServicesTestRunner.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinTecsMerchantServicesTestRunner.getenv(key)
        ...
        explain = BluefinTecsMerchantServicesTestRunner.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinTecsMerchantServicesTestRunner._test_control is not None:
            return BluefinTecsMerchantServicesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinTecsMerchantServicesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinTecsMerchantServicesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinTecsMerchantServicesTestRunner.load_test_control()
...
def env_override(m):
    return BluefinTecsMerchantServicesTestRunner.env_override(m)


def entity_data(v):
    return BluefinTecsMerchantServicesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinTecsMerchantServicesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinTecsMerchantServicesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinTecsMerchantServicesTestRunner.load_test_control()


def live_client_options():
    return BluefinTecsMerchantServicesTestRunner.live_client_options()


def live_delay_ms():
    return BluefinTecsMerchantServicesTestRunner.live_delay_ms()
