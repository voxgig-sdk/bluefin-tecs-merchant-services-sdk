...
class TestEcDataEcomEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.EcDataEcom(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _ec_data_ecom_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "ec_data_ecom." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        ec_data_ecom_ref01_ent = client.EcDataEcom(None)
        ec_data_ecom_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.ec_data_ecom"), "ec_data_ecom_ref01"))

        ec_data_ecom_ref01_data = helpers.to_map(ec_data_ecom_ref01_ent.create(ec_data_ecom_ref01_data, None))
        assert ec_data_ecom_ref01_data is not None



def _ec_data_ecom_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/ec_data_ecom/EcDataEcomTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID"))
