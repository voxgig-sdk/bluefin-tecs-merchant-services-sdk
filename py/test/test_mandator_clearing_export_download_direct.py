...
class TestMandatorClearingExportDownloadDirect:

    def test_should_direct_load_mandator_clearing_export_download(self):
        setup = _mandator_clearing_export_download_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-mandator_clearing_export_download", "live" if setup["live"] else "unit")
        if _skip:
...
def _mandator_clearing_export_download_direct_setup(mockres):
