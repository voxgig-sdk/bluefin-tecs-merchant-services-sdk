...
class TestDigitalServicesApiEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.DigitalServicesApi(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _digital_services_api_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "digital_services_api." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_DIGITAL_SERVICES_API_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        digital_services_api_ref01_ent = client.DigitalServicesApi(None)
        ...
        digital_services_api_ref01_data = helpers.to_map(runner.entity_data(digital_services_api_ref01_ent.create(digital_services_api_ref01_data, None)))
        assert digital_services_api_ref01_data is not None

        # LOAD
        digital_services_api_ref01_match_dt0 = {}
        digital_services_api_ref01_data_dt0_loaded = digital_services_api_ref01_ent.load(digital_services_api_ref01_match_dt0, None)
        assert digital_services_api_ref01_data_dt0_loaded is not None



def _digital_services_api_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/digital_services_api/DigitalServicesApiTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_DIGITAL_SERVICES_API_ENTID"))
