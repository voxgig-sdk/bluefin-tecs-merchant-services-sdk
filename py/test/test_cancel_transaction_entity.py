...
class TestCancelTransactionEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.CancelTransaction(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _cancel_transaction_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "cancel_transaction." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_CANCEL_TRANSACTION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        cancel_transaction_ref01_ent = client.CancelTransaction(None)
        ...
        cancel_transaction_ref01_data = helpers.to_map(runner.entity_data(cancel_transaction_ref01_ent.create(cancel_transaction_ref01_data, None)))
        assert cancel_transaction_ref01_data is not None



def _cancel_transaction_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/cancel_transaction/CancelTransactionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_CANCEL_TRANSACTION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_CANCEL_TRANSACTION_ENTID"))
