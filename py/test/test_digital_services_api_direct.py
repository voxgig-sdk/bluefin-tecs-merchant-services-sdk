...
class TestDigitalServicesApiDirect:

    def test_should_direct_load_digital_services_api(self):
        setup = _digital_services_api_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-digital_services_api", "live" if setup["live"] else "unit")
        if _skip:
...
def _digital_services_api_direct_setup(mockres):
