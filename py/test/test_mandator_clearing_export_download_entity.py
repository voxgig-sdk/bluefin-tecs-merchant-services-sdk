...
class TestMandatorClearingExportDownloadEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.MandatorClearingExportDownload(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _mandator_clearing_export_download_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "mandator_clearing_export_download." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        mandator_clearing_export_download_ref01_ent = client.MandatorClearingExportDownload(None)
        ...
        mandator_clearing_export_download_ref01_data = helpers.to_map(mandator_clearing_export_download_ref01_ent.create(mandator_clearing_export_download_ref01_data, None))
...
def _mandator_clearing_export_download_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/mandator_clearing_export_download/MandatorClearingExportDownloadTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID"))
