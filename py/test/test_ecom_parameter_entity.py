...
class TestEcomParameterEntity:

    def test_should_create_instance(self):
        testsdk = BluefinTecsMerchantServicesSDK.test(None, None)
        ent = testsdk.EcomParameter(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _ecom_parameter_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "ecom_parameter." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        ecom_parameter_ref01_ent = client.EcomParameter(None)
        ecom_parameter_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.ecom_parameter"), "ecom_parameter_ref01"))

        ecom_parameter_ref01_data = helpers.to_map(ecom_parameter_ref01_ent.create(ecom_parameter_ref01_data, None))
        assert ecom_parameter_ref01_data is not None



def _ecom_parameter_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/ecom_parameter/EcomParameterTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID"))
