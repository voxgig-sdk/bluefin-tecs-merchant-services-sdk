    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ECR_DATA_ENTID JSON to run live")
        client = setup["client"]
        ...
        ecr_data_ref01_data = helpers.to_map(runner.entity_data(ecr_data_ref01_ent.create(ecr_data_ref01_data, None)))
...
def _ecr_data_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ECR_DATA_ENTID"))
