...
class TestTerminalIdEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.TerminalId(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _terminal_id_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "terminal_id." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TERMINAL_ID_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        terminal_id_ref01_ent = client.TerminalId(None)
        terminal_id_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.terminal_id"), "terminal_id_ref01"))

        terminal_id_ref01_data = helpers.to_map(runner.entity_data(terminal_id_ref01_ent.create(terminal_id_ref01_data, None)))
        assert terminal_id_ref01_data is not None



def _terminal_id_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/terminal_id/TerminalIdTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TERMINAL_ID_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TERMINAL_ID_ENTID"))
