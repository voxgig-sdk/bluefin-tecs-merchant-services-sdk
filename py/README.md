...
| Field | Type | Description |
| --- | --- | --- |
| `clearingDateFrom` | `str` | Date and time in the format yyyy-MM-dd'T'HH:mm:ssz |
| `clearingDateTo` | `str` | Date and time in the format yyyy-MM-dd'T'HH:mm:ssz |
| `responseCode` | `int` |  |
| `responseMessage` | `str` |  |
| `txCount` | `int` |  |
| `txIdEnd` | `str` |  |
...
| Field | Type | Description |
| --- | --- | --- |
| `clearingDateFrom` | `str` | Start date for clearing export (inclusive) |
| `clearingDateTo` | `str` | End date for clearing export (inclusive) |
| `fileId` | `str` | Unique file identifier for tracking and downloading |
| `filenameTemplate` | `str` | Optional filename template for the export file |
| `id` | `str` |  |
| `responseCode` | `int` |  |
| `responseMessage` | `str` |  |
| `status` | `str` | Processing status of the export request |
...
| `transactionId` | `str` |  |
| `transactionType` | `str` |  |
| `wallet` | `str` | Filter by wallet type. |
...
| Field | Type | Description |
| --- | --- | --- |
| `acquirerName` | `str` | Acquirer name parsed from KKG field |
| `amount` | `int` | Transaction amount in minor units (cents) |
| `authorizationNumber` | `str` | Authorization number from the gateway |
| `cardNumber` | `str` | Card number - 12 to 19 digits, must pass Luhn validation |
| `cardType` | `str` | Card type parsed from KKG field |
| `currency` | `str` | Currency code - 3 uppercase letters (ISO 4217) |
| `cvc` | `str` | Card verification code - 3-4 digits (optional) |
| `dateTimeTx` | `str` | Date and time of the transaction |
| `expDate` | `str` | Card expiry date in MMYY format |
| `merchantId` | `str` | Merchant ID (VU-NUMMER) |
| `originalTransactionId` | `str` | Original transaction ID from gateway |
| `password` | `str` | Terminal password sent as Kennwort in TECS XML (optional) |
| `responseCode` | `str` | Response code - 00 for success, otherwise error code |
| `responseMessage` | `str` | Response message - 'Approved' for success, error description otherwise |
| `terminalId` | `str` | Terminal ID used for the transaction |
| `transactionId` | `str` | Transaction ID generated by the backend |
| `txtype` | `str` | Transaction type |
...
| `amount` | `int` | Transaction amount in minor units (cents) |
| `currency` | `str` | Currency code - 3 uppercase letters (ISO 4217) |
| `device` | `str` | Device type that provided the SRED payload |
| `devicePayload` | `str` | SRED encrypted device payload from the device (minimum 32 characters) |
| `expDate` | `str` | Card expiry date in MMYY format |
| `mode` | `str` | Decryption mode |
| `panMasked` | `str` | Masked PAN (first 6 and last 4 digits) |
| `password` | `str` | Terminal password sent as Kennwort in TECS XML (optional) |
| `serial` | `str` | Device serial number |
| `serviceCode` | `str` | Service code from the card |
| `terminalId` | `str` | Terminal ID - 8 digits |
| `txtype` | `str` | Transaction type |
...
| `clearingDateFrom` | `str` | Date and time in the format yyyy-MM-dd'T'HH:mm:ss |
| `clearingDateTo` | `str` | Date and time in the format yyyy-MM-dd'T'HH:mm:ss |
| `corporateId` | `str` |  |
| `currency` | `str` |  |
| `responseCode` | `int` |  |
| `responseMessage` | `str` |  |
...
| `authorizationCode` | `str | None` | Authorization code returned by the acquirer; null when not available |
