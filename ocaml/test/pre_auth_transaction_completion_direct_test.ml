(* Generated pre_auth_transaction_completion direct-call test. *)
...
let () =
  test "pre_auth_transaction_completion.direct_call" (fun () ->
      let calls = ref [] in
      ...
      let result = Sdk_client.direct client
          (jo [("path", Str "/pre_auth_transaction_completion"); ("method", Str "GET")]) in
