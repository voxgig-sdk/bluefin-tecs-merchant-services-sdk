...
class MandatorClearingExportEntityTest {
  ...
  @Test
  fun basic() {
    val setup = mandatorClearingExportBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "mandator_clearing_export.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val mandatorClearingExportRef01Ent = client.mandatorClearingExport(null)
    var mandatorClearingExportRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.mandator_clearing_export"), "mandator_clearing_export_ref01")) ?: linkedMapOf())

    val mandatorClearingExportRef01DataResult = mandatorClearingExportRef01Ent.create(mandatorClearingExportRef01Data, null)
    mandatorClearingExportRef01Data = Helpers.toMapAny(if (mandatorClearingExportRef01DataResult is SdkEntity) mandatorClearingExportRef01DataResult.data() else mandatorClearingExportRef01DataResult) ?: linkedMapOf()
    assertNotNull(mandatorClearingExportRef01Data, "expected create result to be a map")

  }

  companion object {
    fun mandatorClearingExportBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read mandator_clearing_export test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID"] = idmap
