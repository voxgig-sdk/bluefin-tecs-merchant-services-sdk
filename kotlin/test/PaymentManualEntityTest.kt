...
class PaymentManualEntityTest {
  ...
  @Test
  fun basic() {
    val setup = paymentManualBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "payment_manual.$op", mode)
    ...
    var paymentManualRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.payment_manual"), "payment_manual_ref01")) ?: linkedMapOf())

    val paymentManualRef01DataResult = paymentManualRef01Ent.create(paymentManualRef01Data, null)
    paymentManualRef01Data = Helpers.toMapAny(if (paymentManualRef01DataResult is SdkEntity) paymentManualRef01DataResult.data() else paymentManualRef01DataResult) ?: linkedMapOf()
    assertNotNull(paymentManualRef01Data, "expected create result to be a map")

  }

  companion object {
    fun paymentManualBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read payment_manual test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PAYMENT_MANUAL_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PAYMENT_MANUAL_ENTID"] = idmap
