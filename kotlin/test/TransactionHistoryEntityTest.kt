...
class TransactionHistoryEntityTest {
  ...
  @Test
  fun basic() {
    val setup = transactionHistoryBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "transaction_history.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTION_HISTORY_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val transactionHistoryRef01Ent = client.transactionHistory(null)
    var transactionHistoryRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.transaction_history"), "transaction_history_ref01")) ?: linkedMapOf())

    val transactionHistoryRef01DataResult = transactionHistoryRef01Ent.create(transactionHistoryRef01Data, null)
    transactionHistoryRef01Data = Helpers.toMapAny(if (transactionHistoryRef01DataResult is SdkEntity) transactionHistoryRef01DataResult.data() else transactionHistoryRef01DataResult) ?: linkedMapOf()
    assertNotNull(transactionHistoryRef01Data, "expected create result to be a map")

  }

  companion object {
    fun transactionHistoryBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read transaction_history test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTION_HISTORY_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTION_HISTORY_ENTID"] = idmap
