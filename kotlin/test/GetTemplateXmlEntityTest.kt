...
class GetTemplateXmlEntityTest {
  ...
  @Test
  fun basic() {
    val setup = getTemplateXmlBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "get_template_xml.$op", mode)
    ...
    var getTemplateXmlRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.get_template_xml"), "get_template_xml_ref01")) ?: linkedMapOf())

    val getTemplateXmlRef01DataResult = getTemplateXmlRef01Ent.create(getTemplateXmlRef01Data, null)
    getTemplateXmlRef01Data = Helpers.toMapAny(if (getTemplateXmlRef01DataResult is SdkEntity) getTemplateXmlRef01DataResult.data() else getTemplateXmlRef01DataResult) ?: linkedMapOf()
    assertNotNull(getTemplateXmlRef01Data, "expected create result to be a map")

  }

  companion object {
    fun getTemplateXmlBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read get_template_xml test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_GET_TEMPLATE_XML_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_GET_TEMPLATE_XML_ENTID"] = idmap
