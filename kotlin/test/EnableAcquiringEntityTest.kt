...
class EnableAcquiringEntityTest {
  ...
  @Test
  fun basic() {
    val setup = enableAcquiringBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "enable_acquiring.$op", mode)
    ...
    var enableAcquiringRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.enable_acquiring"), "enable_acquiring_ref01")) ?: linkedMapOf())

    val enableAcquiringRef01DataResult = enableAcquiringRef01Ent.create(enableAcquiringRef01Data, null)
    enableAcquiringRef01Data = Helpers.toMapAny(if (enableAcquiringRef01DataResult is SdkEntity) enableAcquiringRef01DataResult.data() else enableAcquiringRef01DataResult) ?: linkedMapOf()
    assertNotNull(enableAcquiringRef01Data, "expected create result to be a map")

  }

  companion object {
    fun enableAcquiringBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read enable_acquiring test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID"] = idmap
