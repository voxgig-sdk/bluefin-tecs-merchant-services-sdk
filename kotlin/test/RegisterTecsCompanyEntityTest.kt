...
class RegisterTecsCompanyEntityTest {
  ...
  @Test
  fun basic() {
    val setup = registerTecsCompanyBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf<String>("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "register_tecs_company.$op", mode)
    ...
    var registerTecsCompanyRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
  ...
  companion object {
    fun registerTecsCompanyBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read register_tecs_company test data: " + e.message, e)
      }
