...
class EcrDataEntityTest {
  ...
  @Test
  fun basic() {
    val setup = ecrDataBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "ecr_data.$op", mode)
    ...
    var ecrDataRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.ecr_data"), "ecr_data_ref01")) ?: linkedMapOf())

    val ecrDataRef01DataResult = ecrDataRef01Ent.create(ecrDataRef01Data, null)
    ecrDataRef01Data = Helpers.toMapAny(ecrDataRef01DataResult) ?: linkedMapOf()
    assertNotNull(ecrDataRef01Data, "expected create result to be a map")

  }

  companion object {
    fun ecrDataBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read ecr_data test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFINTECSMERCHANTSERVICES_TEST_ECR_DATA_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFINTECSMERCHANTSERVICES_TEST_ECR_DATA_ENTID"] = idmap
      ...
      var idmapResolved = Helpers.toMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_ECR_DATA_ENTID"])
