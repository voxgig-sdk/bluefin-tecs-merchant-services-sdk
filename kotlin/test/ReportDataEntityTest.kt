...
class ReportDataEntityTest {
  ...
  @Test
  fun basic() {
    val setup = reportDataBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "report_data.$op", mode)
    ...
    var reportDataRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.report_data"), "report_data_ref01")) ?: linkedMapOf())

    val reportDataRef01DataResult = reportDataRef01Ent.create(reportDataRef01Data, null)
    reportDataRef01Data = Helpers.toMapAny(reportDataRef01DataResult) ?: linkedMapOf()
    assertNotNull(reportDataRef01Data, "expected create result to be a map")

  }

  companion object {
    fun reportDataBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read report_data test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID"] = idmap
      ...
      var idmapResolved = Helpers.toMapAny(env["BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID"])
