...
class VersionDirectTest {

  @Test
  fun directLoadVersion() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-version", mode)
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_VERSION_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_VERSION_ENTID"]
