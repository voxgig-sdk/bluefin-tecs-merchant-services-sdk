...
// BluefinTecsMerchantServices SDK utility: result_basic

class BluefinTecsMerchantServicesResultBasic
{
    public static function call(BluefinTecsMerchantServicesContext $ctx): ?BluefinTecsMerchantServicesResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof BluefinTecsMerchantServicesError) ? $result->err->msg : (string)$result->err;
