...
class DigitalServicesApiEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BluefinTecsMerchantServicesSDK::test(null, null);
        $ent = $testsdk->DigitalServicesApi(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = digital_services_api_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "digital_services_api." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $digital_services_api_ref01_ent = $client->DigitalServicesApi(null);
        ...
        $digital_services_api_ref01_data_dt0_loaded = $digital_services_api_ref01_ent->load($digital_services_api_ref01_match_dt0, null);
        $this->assertNotNull($digital_services_api_ref01_data_dt0_loaded);
...
function digital_services_api_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/digital_services_api/DigitalServicesApiTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["digital_services_api01", "digital_services_api02", "digital_services_api03", "mandator_clearing_export_download01", "mandator_clearing_export_download02", "mandator_clearing_export_download03"] as $k) {
