...
class MandatorClearingExportSummaryEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BluefinTecsMerchantServicesSDK::test(null, null);
        $ent = $testsdk->MandatorClearingExportSummary(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = mandator_clearing_export_summary_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "mandator_clearing_export_summary." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $mandator_clearing_export_summary_ref01_ent = $client->MandatorClearingExportSummary(null);
...
function mandator_clearing_export_summary_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/mandator_clearing_export_summary/MandatorClearingExportSummaryTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["mandator_clearing_export_summary01", "mandator_clearing_export_summary02", "mandator_clearing_export_summary03"] as $k) {
