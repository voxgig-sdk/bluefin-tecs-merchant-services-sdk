...
class EnableAcquiringEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $enable_acquiring_ref01_data = Helpers::to_map(is_object($enable_acquiring_ref01_data_result) && method_exists($enable_acquiring_ref01_data_result, 'data_get') ? $enable_acquiring_ref01_data_result->data_get() : $enable_acquiring_ref01_data_result);
...
function enable_acquiring_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID");
