...
class EcDataEcomEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_EC_DATA_ECOM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ec_data_ecom_ref01_data = Helpers::to_map(is_object($ec_data_ecom_ref01_data_result) && method_exists($ec_data_ecom_ref01_data_result, 'data_get') ? $ec_data_ecom_ref01_data_result->data_get() : $ec_data_ecom_ref01_data_result);
...
function ec_data_ecom_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_EC_DATA_ECOM_ENTID");
