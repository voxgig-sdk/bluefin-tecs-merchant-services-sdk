...
class TransactionsTurnoverEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTIONS_TURNOVER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $transactions_turnover_ref01_data = Helpers::to_map(is_object($transactions_turnover_ref01_data_result) && method_exists($transactions_turnover_ref01_data_result, 'data_get') ? $transactions_turnover_ref01_data_result->data_get() : $transactions_turnover_ref01_data_result);
...
function transactions_turnover_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TRANSACTIONS_TURNOVER_ENTID");
