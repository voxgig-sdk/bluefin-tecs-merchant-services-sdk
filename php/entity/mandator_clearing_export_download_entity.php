...
// BluefinTecsMerchantServices SDK MandatorClearingExportDownload entity
...
class MandatorClearingExportDownloadEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "mandator_clearing_export_download";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new MandatorClearingExportDownloadEntity($this->_client, $opts);
    }

    /**
     * @param MandatorClearingExportDownload|array $args MandatorClearingExportDownload data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinTecsMerchantServicesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return MandatorClearingExportDownload|array The current MandatorClearingExportDownload data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of MandatorClearingExportDownload fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinTecsMerchantServicesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of MandatorClearingExportDownload fields).
     */
    ...
    /**
     * Load a single MandatorClearingExportDownload.
     *
     * @param MandatorClearingExportDownloadLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed MandatorClearingExportDownloadLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return MandatorClearingExportDownload|array The loaded MandatorClearingExportDownload as an assoc-array at the
     *   SDK boundary; throws BluefinTecsMerchantServicesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BluefinTecsMerchantServicesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * Create a new MandatorClearingExportDownload.
     *
     * @param MandatorClearingExportDownloadCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed MandatorClearingExportDownloadCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return MandatorClearingExportDownload|array The created MandatorClearingExportDownload as an assoc-array at the
     *   SDK boundary; throws BluefinTecsMerchantServicesError on failure (item-5 convention).
     */
