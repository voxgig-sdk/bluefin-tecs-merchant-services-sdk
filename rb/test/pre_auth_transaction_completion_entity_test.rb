...
class PreAuthTransactionCompletionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    pre_auth_transaction_completion_ref01_data = Helpers.to_map(pre_auth_transaction_completion_ref01_data_result.respond_to?(:data_get) ? pre_auth_transaction_completion_ref01_data_result.data_get : pre_auth_transaction_completion_ref01_data_result)
...
def pre_auth_transaction_completion_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID"]
