...
class VersionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    version_ref01_data = nil
    if version_ref01_data_raw.length > 0
    ...
    assert !version_ref01_data_dt0_loaded.nil?
...
def version_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID"]
