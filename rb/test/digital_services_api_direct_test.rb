...
class DigitalServicesApiDirectTest < Minitest::Test
  def test_direct_load_digital_services_api
    setup = digital_services_api_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-digital_services_api", setup[:live] ? "live" : "unit")
    if _should_skip
...
def digital_services_api_direct_setup(mockres)
