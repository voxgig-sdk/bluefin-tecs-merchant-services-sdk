...
class MandatorClearingExportEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    mandator_clearing_export_ref01_data = Helpers.to_map(mandator_clearing_export_ref01_data_result.respond_to?(:data_get) ? mandator_clearing_export_ref01_data_result.data_get : mandator_clearing_export_ref01_data_result)
...
def mandator_clearing_export_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID"]
