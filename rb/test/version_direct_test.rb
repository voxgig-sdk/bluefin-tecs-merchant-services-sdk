...
class VersionDirectTest < Minitest::Test
  def test_direct_load_version
    setup = version_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-version", setup[:live] ? "live" : "unit")
    if _should_skip
...
def version_direct_setup(mockres)
