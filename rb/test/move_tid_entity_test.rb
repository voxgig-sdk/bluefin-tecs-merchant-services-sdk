...
class MoveTidEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MOVE_TID_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    move_tid_ref01_data = Helpers.to_map(move_tid_ref01_data_result.respond_to?(:data_get) ? move_tid_ref01_data_result.data_get : move_tid_ref01_data_result)
...
def move_tid_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MOVE_TID_ENTID"]
