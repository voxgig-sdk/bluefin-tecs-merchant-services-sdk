...
class EnableAcquiringEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    enable_acquiring_ref01_data = Helpers.to_map(enable_acquiring_ref01_data_result.respond_to?(:data_get) ? enable_acquiring_ref01_data_result.data_get : enable_acquiring_ref01_data_result)
...
def enable_acquiring_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ENABLE_ACQUIRING_ENTID"]
