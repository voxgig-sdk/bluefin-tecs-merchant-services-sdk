...
class EcDataEcomEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = ec_data_ecom_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "ec_data_ecom." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    ec_data_ecom_ref01_ent = client.EcDataEcom(nil)
...
def ec_data_ecom_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ec_data_ecom", "EcDataEcomTestData.json")
