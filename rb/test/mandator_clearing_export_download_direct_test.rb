...
class MandatorClearingExportDownloadDirectTest < Minitest::Test
  def test_direct_load_mandator_clearing_export_download
    setup = mandator_clearing_export_download_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-mandator_clearing_export_download", setup[:live] ? "live" : "unit")
    if _should_skip
...
def mandator_clearing_export_download_direct_setup(mockres)
