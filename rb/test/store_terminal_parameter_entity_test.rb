...
class StoreTerminalParameterEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    store_terminal_parameter_ref01_data = Helpers.to_map(store_terminal_parameter_ref01_data_result.respond_to?(:data_get) ? store_terminal_parameter_ref01_data_result.data_get : store_terminal_parameter_ref01_data_result)
...
def store_terminal_parameter_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID"]
