...
class MerchantPortalServicesApiEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    merchant_portal_services_api_ref01_data = Helpers.to_map(merchant_portal_services_api_ref01_data_result.respond_to?(:data_get) ? merchant_portal_services_api_ref01_data_result.data_get : merchant_portal_services_api_ref01_data_result)
...
def merchant_portal_services_api_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID"]
