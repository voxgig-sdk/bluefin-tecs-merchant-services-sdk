...
class MandatorClearingExportDownloadEntity
  ...
  def make
    opts = @_entopts.dup
    MandatorClearingExportDownloadEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinTecsMerchantServicesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [MandatorClearingExportDownload, Hash] the current MandatorClearingExportDownload data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinTecsMerchantServicesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of MandatorClearingExportDownload fields)
  def match_get
  ...
  #
  # @param reqmatch [MandatorClearingExportDownloadLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.MandatorClearingExportDownload.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [MandatorClearingExportDownload, Hash] the loaded MandatorClearingExportDownload; raises BluefinTecsMerchantServicesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinTecsMerchantServicesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqdata [MandatorClearingExportDownloadCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [MandatorClearingExportDownload, Hash] the created MandatorClearingExportDownload; raises BluefinTecsMerchantServicesError on failure
