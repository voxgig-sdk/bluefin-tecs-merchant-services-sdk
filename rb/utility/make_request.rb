...
module BluefinTecsMerchantServicesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinTecsMerchantServicesResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinTecsMerchantServicesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinTecsMerchantServicesResponse.new(fetched)
    else
