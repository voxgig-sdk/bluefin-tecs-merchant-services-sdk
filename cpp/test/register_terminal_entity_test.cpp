...
static RegisterTerminalSetup register_terminal_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REGISTER_TERMINAL_ENTID"));
...
static void register_terminal_entity_basic() {
  ...
  {
    Value register_terminal_ref01_data_result = register_terminal_ref01_ent->create(Struct::clone(register_terminal_ref01_data), Value::undef())->data();
