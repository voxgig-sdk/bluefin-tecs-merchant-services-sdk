...
static CreateProductSetup create_product_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_CREATE_PRODUCT_ENTID"));
...
static void create_product_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"create_product", vmap({
...
static void create_product_entity_basic() {
  ...
  {
    Value create_product_ref01_data_result = create_product_ref01_ent->create(Struct::clone(create_product_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(create_product_entity_stream);
