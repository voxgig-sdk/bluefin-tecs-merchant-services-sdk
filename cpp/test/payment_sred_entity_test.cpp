...
static PaymentSredSetup payment_sred_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID"));
...
static void payment_sred_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"payment_sred", vmap({
...
static void payment_sred_entity_basic() {
  ...
  {
    Value payment_sred_ref01_data_result = payment_sred_ref01_ent->create(Struct::clone(payment_sred_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(payment_sred_entity_stream);
