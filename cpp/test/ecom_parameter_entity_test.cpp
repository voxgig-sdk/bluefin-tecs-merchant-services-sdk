...
static EcomParameterSetup ecom_parameter_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID"));
...
static void ecom_parameter_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"ecom_parameter", vmap({
...
static void ecom_parameter_entity_basic() {
  ...
  {
    Value ecom_parameter_ref01_data_result = ecom_parameter_ref01_ent->create(Struct::clone(ecom_parameter_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(ecom_parameter_entity_stream);
