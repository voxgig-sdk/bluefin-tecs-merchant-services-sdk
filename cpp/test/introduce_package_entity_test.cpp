...
static IntroducePackageSetup introduce_package_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_INTRODUCE_PACKAGE_ENTID"));
...
static void introduce_package_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"introduce_package", vmap({
...
static void introduce_package_entity_basic() {
  ...
  {
    Value introduce_package_ref01_data_result = introduce_package_ref01_ent->create(Struct::clone(introduce_package_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(introduce_package_entity_stream);
