...
static ListTerminalSetup list_terminal_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_LIST_TERMINAL_ENTID"));
...
static void list_terminal_entity_basic() {
  ...
  {
    Value list_terminal_ref01_data_result = list_terminal_ref01_ent->create(Struct::clone(list_terminal_ref01_data), Value::undef())->data();
