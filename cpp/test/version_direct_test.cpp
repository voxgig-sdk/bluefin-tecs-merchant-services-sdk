// Generated direct-call tests for the version entity (unit mode;
...
static VersionDirectSetup version_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void version_direct_load() {
  auto setup = version_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-version", "unit");
...
int main() {
  T_RUN(version_direct_load);
  return sdktest::summary("version_direct_test");
}
