...
static UpdateMerchantSetup update_merchant_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_UPDATE_MERCHANT_ENTID"));
...
static void update_merchant_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"update_merchant", vmap({
...
static void update_merchant_entity_basic() {
  ...
  {
    Value update_merchant_ref01_data_result = update_merchant_ref01_ent->create(Struct::clone(update_merchant_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(update_merchant_entity_stream);
