// BluefinTecsMerchantServices SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinTecsMerchantServicesSDK> plClient(const Value& sdkopts) {
  return BluefinTecsMerchantServicesSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinTecsMerchantServicesSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinTecsMerchantServicesSDK> client) {
  std::string out;
