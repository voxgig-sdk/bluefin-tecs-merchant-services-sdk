// Generated direct-call tests for the mandator_clearing_export_download entity (unit mode;
...
static MandatorClearingExportDownloadDirectSetup mandator_clearing_export_download_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  auto client = std::make_shared<BluefinTecsMerchantServicesSDK>(opts);

  MandatorClearingExportDownloadDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void mandator_clearing_export_download_direct_load() {
  auto setup = mandator_clearing_export_download_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-mandator_clearing_export_download", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("public/digitalservices/mandatorClearingExportDownload/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(mandator_clearing_export_download_direct_load);
  return sdktest::summary("mandator_clearing_export_download_direct_test");
}
