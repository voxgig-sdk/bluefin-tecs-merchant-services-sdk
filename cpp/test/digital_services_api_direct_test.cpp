// Generated direct-call tests for the digital_services_api entity (unit mode;
...
static DigitalServicesApiDirectSetup digital_services_api_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  DigitalServicesApiDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void digital_services_api_direct_load() {
  auto setup = digital_services_api_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-digital_services_api", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("public/digitalservices/mandatorClearingExportDownload/status")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(digital_services_api_direct_load);
  return sdktest::summary("digital_services_api_direct_test");
}
