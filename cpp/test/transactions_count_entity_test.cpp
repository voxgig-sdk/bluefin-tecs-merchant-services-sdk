...
static TransactionsCountSetup transactions_count_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTIONS_COUNT_ENTID"));
...
static void transactions_count_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"transactions_count", vmap({
...
static void transactions_count_entity_basic() {
  ...
  {
    Value transactions_count_ref01_data_result = transactions_count_ref01_ent->create(Struct::clone(transactions_count_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(transactions_count_entity_stream);
