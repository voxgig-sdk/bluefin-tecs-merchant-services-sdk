...
static EcDataEcomSetup ec_data_ecom_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID"));
...
static void ec_data_ecom_entity_stream() {
  // stream() runs the list op through the full pipeline and returns the
  // result items. Seed two entities via test mode; with the streaming feature
  // active it yields the feature's incremental items, else it falls back to
  // the materialised items — either way every item is yielded.
  Value seed = vmap({{"entity", vmap({{"ec_data_ecom", vmap({
...
static void ec_data_ecom_entity_basic() {
  ...
  {
    Value ec_data_ecom_ref01_data_result = ec_data_ecom_ref01_ent->create(Struct::clone(ec_data_ecom_ref01_data), Value::undef());
...
int main() {
  ...
  T_RUN(ec_data_ecom_entity_stream);
