...
static GetTemplateXmlSetup get_template_xml_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/get_template_xml/GetTemplateXmlTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("get_template_xml01"), Value("get_template_xml02"), Value("get_template_xml03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void get_template_xml_entity_instance() {
  auto testsdk = BluefinTecsMerchantServicesSDK::testSDK();
  auto ent = testsdk->get_template_xml();
...
static void get_template_xml_entity_basic() {
  ...
  for (const std::string& op : std::vector<std::string>{"create"}) {
    auto sk = is_control_skipped("entityOp", std::string("get_template_xml.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto get_template_xml_ref01_ent = client->get_template_xml();
