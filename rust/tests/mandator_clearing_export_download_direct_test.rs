// Generated direct-call tests for the mandator_clearing_export_download entity (mirrors the
...
use bluefin_tecs_merchant_services_sdk::core::helpers::{getp, ja, jo, json_thunk, setp, to_int, to_map};
use bluefin_tecs_merchant_services_sdk::utility::voxgigstruct as vs;
use bluefin_tecs_merchant_services_sdk::{Value, BluefinTecsMerchantServicesSDK};

struct MandatorClearingExportDownloadDirectSetup {
...
fn mandator_clearing_export_download_direct_setup(mockres: Value) -> MandatorClearingExportDownloadDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return MandatorClearingExportDownloadDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    MandatorClearingExportDownloadDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn mandator_clearing_export_download_direct_load() {
    let setup = mandator_clearing_export_download_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-mandator_clearing_export_download", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["mandator_clearing_export_download01"] {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "mandator_clearing_export_download01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("public/digitalservices/mandatorClearingExportDownload/{id}")),
