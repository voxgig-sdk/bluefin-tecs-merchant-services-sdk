...
#[test]
fn reactivate_terminal_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.reactivate_terminal(Value::Noval);
    assert_eq!(ent.get_name(), "reactivate_terminal");
}

#[test]
fn reactivate_terminal_entity_stream() {
    ...
    let plainent = plainsdk.reactivate_terminal(Value::Noval);
...
#[test]
fn reactivate_terminal_entity_basic() {
    let setup = reactivate_terminal_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("reactivate_terminal.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_REACTIVATE_TERMINAL_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let reactivate_terminal_ref01_ent = client.reactivate_terminal(Value::Noval);
    let reactivate_terminal_ref01_data = to_map(&getp(
        &getpath(&["new", "reactivate_terminal"], &setup.data),
        "reactivate_terminal_ref01",
    ));

    let reactivate_terminal_ref01_data_result = reactivate_terminal_ref01_ent
        .create(reactivate_terminal_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let reactivate_terminal_ref01_data = to_map(&reactivate_terminal_ref01_data_result);
    assert!(
        matches!(reactivate_terminal_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn reactivate_terminal_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("reactivate_terminal");
    entity_data_file.push("ReactivateTerminalTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("reactivate_terminal01"), Value::str("reactivate_terminal02"), Value::str("reactivate_terminal03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_REACTIVATE_TERMINAL_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_REACTIVATE_TERMINAL_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_REACTIVATE_TERMINAL_ENTID")) {
