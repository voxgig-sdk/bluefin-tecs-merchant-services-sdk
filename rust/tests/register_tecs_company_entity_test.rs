...
#[test]
fn register_tecs_company_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.register_tecs_company(Value::Noval);
    assert_eq!(ent.get_name(), "register_tecs_company");
}

#[test]
fn register_tecs_company_entity_stream() {
    ...
    let plainent = plainsdk.register_tecs_company(Value::Noval);
...
#[test]
fn register_tecs_company_entity_basic() {
    let setup = register_tecs_company_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("register_tecs_company.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let register_tecs_company_ref01_ent = client.register_tecs_company(Value::Noval);
    let register_tecs_company_ref01_data = to_map(&getp(
        &getpath(&["new", "register_tecs_company"], &setup.data),
        "register_tecs_company_ref01",
    ));

    let register_tecs_company_ref01_data_result = register_tecs_company_ref01_ent
        .create(register_tecs_company_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let register_tecs_company_ref01_data = to_map(&register_tecs_company_ref01_data_result);
    assert!(
        matches!(register_tecs_company_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn register_tecs_company_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("register_tecs_company");
    entity_data_file.push("RegisterTecsCompanyTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("register_tecs_company01"), Value::str("register_tecs_company02"), Value::str("register_tecs_company03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID")) {
