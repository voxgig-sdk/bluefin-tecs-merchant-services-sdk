...
#[test]
fn update_merchant_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.update_merchant(Value::Noval);
    assert_eq!(ent.get_name(), "update_merchant");
}

#[test]
fn update_merchant_entity_stream() {
    ...
    let plainent = plainsdk.update_merchant(Value::Noval);
...
#[test]
fn update_merchant_entity_basic() {
    let setup = update_merchant_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("update_merchant.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_UPDATE_MERCHANT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let update_merchant_ref01_ent = client.update_merchant(Value::Noval);
    let update_merchant_ref01_data = to_map(&getp(
        &getpath(&["new", "update_merchant"], &setup.data),
        "update_merchant_ref01",
    ));

    let update_merchant_ref01_data_result = update_merchant_ref01_ent
        .create(update_merchant_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let update_merchant_ref01_data = to_map(&update_merchant_ref01_data_result);
    assert!(
        matches!(update_merchant_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn update_merchant_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("update_merchant");
    entity_data_file.push("UpdateMerchantTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("update_merchant01"), Value::str("update_merchant02"), Value::str("update_merchant03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_UPDATE_MERCHANT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_UPDATE_MERCHANT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_UPDATE_MERCHANT_ENTID")) {
