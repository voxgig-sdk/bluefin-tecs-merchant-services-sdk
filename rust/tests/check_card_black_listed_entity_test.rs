...
#[test]
fn check_card_black_listed_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.check_card_black_listed(Value::Noval);
    assert_eq!(ent.get_name(), "check_card_black_listed");
}

#[test]
fn check_card_black_listed_entity_stream() {
    ...
    let plainent = plainsdk.check_card_black_listed(Value::Noval);
...
#[test]
fn check_card_black_listed_entity_basic() {
    let setup = check_card_black_listed_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("check_card_black_listed.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let check_card_black_listed_ref01_ent = client.check_card_black_listed(Value::Noval);
    let check_card_black_listed_ref01_data = to_map(&getp(
        &getpath(&["new", "check_card_black_listed"], &setup.data),
        "check_card_black_listed_ref01",
    ));

    let check_card_black_listed_ref01_data_result = check_card_black_listed_ref01_ent
        .create(check_card_black_listed_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let check_card_black_listed_ref01_data = to_map(&check_card_black_listed_ref01_data_result);
    assert!(
        matches!(check_card_black_listed_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn check_card_black_listed_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("check_card_black_listed");
    entity_data_file.push("CheckCardBlackListedTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("check_card_black_listed01"), Value::str("check_card_black_listed02"), Value::str("check_card_black_listed03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID")) {
