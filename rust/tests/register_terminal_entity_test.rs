...
#[test]
fn register_terminal_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.register_terminal(Value::Noval);
    assert_eq!(ent.get_name(), "register_terminal");
}

#[test]
fn register_terminal_entity_stream() {
    ...
    let plainent = plainsdk.register_terminal(Value::Noval);
...
#[test]
fn register_terminal_entity_basic() {
    let setup = register_terminal_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("register_terminal.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TERMINAL_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let register_terminal_ref01_ent = client.register_terminal(Value::Noval);
    let register_terminal_ref01_data = to_map(&getp(
        &getpath(&["new", "register_terminal"], &setup.data),
        "register_terminal_ref01",
    ));

    let register_terminal_ref01_data_result = register_terminal_ref01_ent
        .create(register_terminal_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let register_terminal_ref01_data = to_map(&register_terminal_ref01_data_result);
    assert!(
        matches!(register_terminal_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn register_terminal_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("register_terminal");
    entity_data_file.push("RegisterTerminalTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("register_terminal01"), Value::str("register_terminal02"), Value::str("register_terminal03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TERMINAL_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TERMINAL_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TERMINAL_ENTID")) {
