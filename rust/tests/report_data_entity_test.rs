...
fn report_data_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID JSON to run live");
        return;
    }
    ...
    let report_data_ref01_data = to_map(&report_data_ref01_data_result.data(None));
    assert!(
...
fn report_data_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFIN_TECS_MERCHANT_SERVICES_TEST_REPORT_DATA_ENTID")) {
