...
#[test]
fn report_data_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.report_data(Value::Noval);
    assert_eq!(ent.get_name(), "report_data");
}

#[test]
fn report_data_entity_stream() {
    ...
    let plainent = plainsdk.report_data(Value::Noval);
...
#[test]
fn report_data_entity_basic() {
    let setup = report_data_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("report_data.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let report_data_ref01_ent = client.report_data(Value::Noval);
    let report_data_ref01_data = to_map(&getp(
        &getpath(&["new", "report_data"], &setup.data),
        "report_data_ref01",
    ));

    let report_data_ref01_data_result = report_data_ref01_ent
        .create(report_data_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let report_data_ref01_data = to_map(&report_data_ref01_data_result);
    assert!(
        matches!(report_data_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn report_data_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("report_data");
    entity_data_file.push("ReportDataTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("report_data01"), Value::str("report_data02"), Value::str("report_data03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_REPORT_DATA_ENTID")) {
