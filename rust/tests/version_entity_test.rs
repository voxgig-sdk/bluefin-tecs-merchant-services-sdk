...
#[test]
fn version_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.version(Value::Noval);
    assert_eq!(ent.get_name(), "version");
}

#[test]
fn version_entity_stream() {
    ...
    let plainent = plainsdk.version(Value::Noval);
...
#[test]
fn version_entity_basic() {
    let setup = version_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("version.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let version_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "version"], &setup.data)));
    let version_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&version_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let version_ref01_ent = client.version(Value::Noval);
    let version_ref01_match_dt0 = Value::empty_map();
    let version_ref01_data_dt0_loaded = version_ref01_ent
        .load(version_ref01_match_dt0.clone(), Value::Noval)
        .expect("load failed");
    assert!(
        !version_ref01_data_dt0_loaded.is_noval(),
        "expected load result to be non-nil"
    );

}

fn version_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("version");
    entity_data_file.push("VersionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("version01"), Value::str("version02"), Value::str("version03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID")) {
