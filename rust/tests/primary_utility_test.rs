...
use bluefin_tecs_merchant_services_sdk::core::helpers::{get_str, getp, ja, jo, setp, to_map, vfn};
...
use bluefin_tecs_merchant_services_sdk::{
    ...
    BluefinTecsMerchantServicesError, BluefinTecsMerchantServicesSDK, SdkResult, Spec, Utility, Value,
};
...
where
    F: FnMut(&mut Vec<Value>) -> Result<Value, BluefinTecsMerchantServicesError> + 'static,
{
...
fn base_client() -> (Rc<BluefinTecsMerchantServicesSDK>, Rc<Utility>) {
...
fn make_test_ctx(client: &Rc<BluefinTecsMerchantServicesSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn make_test_full_ctx(client: &Rc<BluefinTecsMerchantServicesSDK>, utility: &Rc<Utility>) -> Rc<Context> {
...
fn primary_fetcher_live() {
    ...
    let c = calls.clone();
    let live_client = BluefinTecsMerchantServicesSDK::new(jo(vec![(
        "system",
        jo(vec![(
            "fetch",
            vfn(move |args| {
...
fn primary_fetcher_blocked_test_mode() {
    ...
    let blocked_client = BluefinTecsMerchantServicesSDK::new(jo(vec![(
        "system",
