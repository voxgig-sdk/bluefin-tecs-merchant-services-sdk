...
fn struct_utility() {
    ...
    fn sdk_check(opts: &Value, ctx: &Value) -> Value {
        ...
        let foo_s = if foo.is_nullish() {
            ...
            bluefin_tecs_merchant_services_sdk::utility::voxgigstruct::value::js_string(&foo)
        };
        let bar = get_path(ctx, &Value::str("meta.bar"), None);
        let bar_s = if bar.is_nullish() {
            "0".to_string()
        } else {
            bluefin_tecs_merchant_services_sdk::utility::voxgigstruct::value::js_string(&bar)
        };
