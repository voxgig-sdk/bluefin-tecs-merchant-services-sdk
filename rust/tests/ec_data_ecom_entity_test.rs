...
#[test]
fn ec_data_ecom_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.ec_data_ecom(Value::Noval);
    assert_eq!(ent.get_name(), "ec_data_ecom");
}

#[test]
fn ec_data_ecom_entity_stream() {
    ...
    let plainent = plainsdk.ec_data_ecom(Value::Noval);
...
#[test]
fn ec_data_ecom_entity_basic() {
    let setup = ec_data_ecom_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("ec_data_ecom.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let ec_data_ecom_ref01_ent = client.ec_data_ecom(Value::Noval);
    let ec_data_ecom_ref01_data = to_map(&getp(
        &getpath(&["new", "ec_data_ecom"], &setup.data),
        "ec_data_ecom_ref01",
    ));

    let ec_data_ecom_ref01_data_result = ec_data_ecom_ref01_ent
        .create(ec_data_ecom_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let ec_data_ecom_ref01_data = to_map(&ec_data_ecom_ref01_data_result);
    assert!(
        matches!(ec_data_ecom_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn ec_data_ecom_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("ec_data_ecom");
    entity_data_file.push("EcDataEcomTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("ec_data_ecom01"), Value::str("ec_data_ecom02"), Value::str("ec_data_ecom03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_EC_DATA_ECOM_ENTID")) {
