...
#[test]
fn status_transaction_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.status_transaction(Value::Noval);
    assert_eq!(ent.get_name(), "status_transaction");
}

#[test]
fn status_transaction_entity_stream() {
    ...
    let plainent = plainsdk.status_transaction(Value::Noval);
...
#[test]
fn status_transaction_entity_basic() {
    let setup = status_transaction_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("status_transaction.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_STATUS_TRANSACTION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let status_transaction_ref01_ent = client.status_transaction(Value::Noval);
    let status_transaction_ref01_data = to_map(&getp(
        &getpath(&["new", "status_transaction"], &setup.data),
        "status_transaction_ref01",
    ));

    let status_transaction_ref01_data_result = status_transaction_ref01_ent
        .create(status_transaction_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let status_transaction_ref01_data = to_map(&status_transaction_ref01_data_result);
    assert!(
        matches!(status_transaction_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn status_transaction_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("status_transaction");
    entity_data_file.push("StatusTransactionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("status_transaction01"), Value::str("status_transaction02"), Value::str("status_transaction03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_STATUS_TRANSACTION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_STATUS_TRANSACTION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_STATUS_TRANSACTION_ENTID")) {
