...
#[test]
fn transactions_count_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.transactions_count(Value::Noval);
    assert_eq!(ent.get_name(), "transactions_count");
}

#[test]
fn transactions_count_entity_stream() {
    ...
    let plainent = plainsdk.transactions_count(Value::Noval);
...
#[test]
fn transactions_count_entity_basic() {
    let setup = transactions_count_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("transactions_count.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTIONS_COUNT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let transactions_count_ref01_ent = client.transactions_count(Value::Noval);
    let transactions_count_ref01_data = to_map(&getp(
        &getpath(&["new", "transactions_count"], &setup.data),
        "transactions_count_ref01",
    ));

    let transactions_count_ref01_data_result = transactions_count_ref01_ent
        .create(transactions_count_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let transactions_count_ref01_data = to_map(&transactions_count_ref01_data_result);
    assert!(
        matches!(transactions_count_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn transactions_count_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("transactions_count");
    entity_data_file.push("TransactionsCountTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("transactions_count01"), Value::str("transactions_count02"), Value::str("transactions_count03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTIONS_COUNT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTIONS_COUNT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTIONS_COUNT_ENTID")) {
