...
#[test]
fn mandator_clearing_export_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.mandator_clearing_export(Value::Noval);
    assert_eq!(ent.get_name(), "mandator_clearing_export");
}

#[test]
fn mandator_clearing_export_entity_stream() {
    ...
    let plainent = plainsdk.mandator_clearing_export(Value::Noval);
...
#[test]
fn mandator_clearing_export_entity_basic() {
    let setup = mandator_clearing_export_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("mandator_clearing_export.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let mandator_clearing_export_ref01_ent = client.mandator_clearing_export(Value::Noval);
    let mandator_clearing_export_ref01_data = to_map(&getp(
        &getpath(&["new", "mandator_clearing_export"], &setup.data),
        "mandator_clearing_export_ref01",
    ));

    let mandator_clearing_export_ref01_data_result = mandator_clearing_export_ref01_ent
        .create(mandator_clearing_export_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let mandator_clearing_export_ref01_data = to_map(&mandator_clearing_export_ref01_data_result);
    assert!(
        matches!(mandator_clearing_export_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn mandator_clearing_export_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("mandator_clearing_export");
    entity_data_file.push("MandatorClearingExportTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("mandator_clearing_export01"), Value::str("mandator_clearing_export02"), Value::str("mandator_clearing_export03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID")) {
