...
#[test]
fn store_terminal_parameter_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.store_terminal_parameter(Value::Noval);
    assert_eq!(ent.get_name(), "store_terminal_parameter");
}

#[test]
fn store_terminal_parameter_entity_stream() {
    ...
    let plainent = plainsdk.store_terminal_parameter(Value::Noval);
...
#[test]
fn store_terminal_parameter_entity_basic() {
    let setup = store_terminal_parameter_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("store_terminal_parameter.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let store_terminal_parameter_ref01_ent = client.store_terminal_parameter(Value::Noval);
    let store_terminal_parameter_ref01_data = to_map(&getp(
        &getpath(&["new", "store_terminal_parameter"], &setup.data),
        "store_terminal_parameter_ref01",
    ));

    let store_terminal_parameter_ref01_data_result = store_terminal_parameter_ref01_ent
        .create(store_terminal_parameter_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let store_terminal_parameter_ref01_data = to_map(&store_terminal_parameter_ref01_data_result);
    assert!(
        matches!(store_terminal_parameter_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn store_terminal_parameter_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("store_terminal_parameter");
    entity_data_file.push("StoreTerminalParameterTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("store_terminal_parameter01"), Value::str("store_terminal_parameter02"), Value::str("store_terminal_parameter03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_STORE_TERMINAL_PARAMETER_ENTID")) {
