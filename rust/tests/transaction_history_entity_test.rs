...
#[test]
fn transaction_history_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.transaction_history(Value::Noval);
    assert_eq!(ent.get_name(), "transaction_history");
}

#[test]
fn transaction_history_entity_stream() {
    ...
    let plainent = plainsdk.transaction_history(Value::Noval);
...
#[test]
fn transaction_history_entity_basic() {
    let setup = transaction_history_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("transaction_history.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let transaction_history_ref01_ent = client.transaction_history(Value::Noval);
    let transaction_history_ref01_data = to_map(&getp(
        &getpath(&["new", "transaction_history"], &setup.data),
        "transaction_history_ref01",
    ));

    let transaction_history_ref01_data_result = transaction_history_ref01_ent
        .create(transaction_history_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let transaction_history_ref01_data = to_map(&transaction_history_ref01_data_result);
    assert!(
        matches!(transaction_history_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn transaction_history_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("transaction_history");
    entity_data_file.push("TransactionHistoryTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("transaction_history01"), Value::str("transaction_history02"), Value::str("transaction_history03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_TRANSACTION_HISTORY_ENTID")) {
