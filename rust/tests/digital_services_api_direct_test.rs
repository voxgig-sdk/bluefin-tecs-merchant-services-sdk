// Generated direct-call tests for the digital_services_api entity (mirrors the
...
struct DigitalServicesApiDirectSetup {
...
fn digital_services_api_direct_setup(mockres: Value) -> DigitalServicesApiDirectSetup {
    load_env_local();
    ...
    if live {
        ...
        return DigitalServicesApiDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn digital_services_api_direct_load() {
    let setup = digital_services_api_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-digital_services_api", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("public/digitalservices/mandatorClearingExportDownload/status")),
