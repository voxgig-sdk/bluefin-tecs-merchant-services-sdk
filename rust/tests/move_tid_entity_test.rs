...
#[test]
fn move_tid_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.move_tid(Value::Noval);
    assert_eq!(ent.get_name(), "move_tid");
}

#[test]
fn move_tid_entity_stream() {
    ...
    let plainent = plainsdk.move_tid(Value::Noval);
...
#[test]
fn move_tid_entity_basic() {
    let setup = move_tid_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("move_tid.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_MOVE_TID_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let move_tid_ref01_ent = client.move_tid(Value::Noval);
    let move_tid_ref01_data = to_map(&getp(
        &getpath(&["new", "move_tid"], &setup.data),
        "move_tid_ref01",
    ));

    let move_tid_ref01_data_result = move_tid_ref01_ent
        .create(move_tid_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let move_tid_ref01_data = to_map(&move_tid_ref01_data_result);
    assert!(
        matches!(move_tid_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn move_tid_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("move_tid");
    entity_data_file.push("MoveTidTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("move_tid01"), Value::str("move_tid02"), Value::str("move_tid03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_MOVE_TID_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_MOVE_TID_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_MOVE_TID_ENTID")) {
