...
#[test]
fn refund_transaction_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.refund_transaction(Value::Noval);
    assert_eq!(ent.get_name(), "refund_transaction");
}

#[test]
fn refund_transaction_entity_stream() {
    ...
    let plainent = plainsdk.refund_transaction(Value::Noval);
...
#[test]
fn refund_transaction_entity_basic() {
    let setup = refund_transaction_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("refund_transaction.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_REFUND_TRANSACTION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let refund_transaction_ref01_ent = client.refund_transaction(Value::Noval);
    let refund_transaction_ref01_data = to_map(&getp(
        &getpath(&["new", "refund_transaction"], &setup.data),
        "refund_transaction_ref01",
    ));

    let refund_transaction_ref01_data_result = refund_transaction_ref01_ent
        .create(refund_transaction_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let refund_transaction_ref01_data = to_map(&refund_transaction_ref01_data_result);
    assert!(
        matches!(refund_transaction_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn refund_transaction_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("refund_transaction");
    entity_data_file.push("RefundTransactionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("refund_transaction01"), Value::str("refund_transaction02"), Value::str("refund_transaction03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_REFUND_TRANSACTION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_REFUND_TRANSACTION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_REFUND_TRANSACTION_ENTID")) {
