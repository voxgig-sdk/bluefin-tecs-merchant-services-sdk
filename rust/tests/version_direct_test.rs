// Generated direct-call tests for the version entity (mirrors the
...
fn version_direct_setup(mockres: Value) -> VersionDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_VERSION_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return VersionDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn version_direct_load() {
    let setup = version_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-version", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
