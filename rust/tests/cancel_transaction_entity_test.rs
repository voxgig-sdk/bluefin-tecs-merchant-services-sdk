...
#[test]
fn cancel_transaction_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.cancel_transaction(Value::Noval);
    assert_eq!(ent.get_name(), "cancel_transaction");
}

#[test]
fn cancel_transaction_entity_stream() {
    ...
    let plainent = plainsdk.cancel_transaction(Value::Noval);
...
#[test]
fn cancel_transaction_entity_basic() {
    let setup = cancel_transaction_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("cancel_transaction.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_CANCEL_TRANSACTION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let cancel_transaction_ref01_ent = client.cancel_transaction(Value::Noval);
    let cancel_transaction_ref01_data = to_map(&getp(
        &getpath(&["new", "cancel_transaction"], &setup.data),
        "cancel_transaction_ref01",
    ));

    let cancel_transaction_ref01_data_result = cancel_transaction_ref01_ent
        .create(cancel_transaction_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let cancel_transaction_ref01_data = to_map(&cancel_transaction_ref01_data_result);
    assert!(
        matches!(cancel_transaction_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn cancel_transaction_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("cancel_transaction");
    entity_data_file.push("CancelTransactionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("cancel_transaction01"), Value::str("cancel_transaction02"), Value::str("cancel_transaction03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_CANCEL_TRANSACTION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_CANCEL_TRANSACTION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_CANCEL_TRANSACTION_ENTID")) {
