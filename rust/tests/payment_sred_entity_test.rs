...
#[test]
fn payment_sred_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.payment_sred(Value::Noval);
    assert_eq!(ent.get_name(), "payment_sred");
}

#[test]
fn payment_sred_entity_stream() {
    ...
    let plainent = plainsdk.payment_sred(Value::Noval);
...
#[test]
fn payment_sred_entity_basic() {
    let setup = payment_sred_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("payment_sred.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let payment_sred_ref01_ent = client.payment_sred(Value::Noval);
    let payment_sred_ref01_data = to_map(&getp(
        &getpath(&["new", "payment_sred"], &setup.data),
        "payment_sred_ref01",
    ));

    let payment_sred_ref01_data_result = payment_sred_ref01_ent
        .create(payment_sred_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let payment_sred_ref01_data = to_map(&payment_sred_ref01_data_result);
    assert!(
        matches!(payment_sred_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn payment_sred_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("payment_sred");
    entity_data_file.push("PaymentSredTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("payment_sred01"), Value::str("payment_sred02"), Value::str("payment_sred03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_PAYMENT_SRED_ENTID")) {
