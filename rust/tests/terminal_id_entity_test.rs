...
#[test]
fn terminal_id_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.terminal_id(Value::Noval);
    assert_eq!(ent.get_name(), "terminal_id");
}

#[test]
fn terminal_id_entity_stream() {
    ...
    let plainent = plainsdk.terminal_id(Value::Noval);
...
#[test]
fn terminal_id_entity_basic() {
    let setup = terminal_id_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("terminal_id.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_TERMINAL_ID_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let terminal_id_ref01_ent = client.terminal_id(Value::Noval);
    let terminal_id_ref01_data = to_map(&getp(
        &getpath(&["new", "terminal_id"], &setup.data),
        "terminal_id_ref01",
    ));

    let terminal_id_ref01_data_result = terminal_id_ref01_ent
        .create(terminal_id_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let terminal_id_ref01_data = to_map(&terminal_id_ref01_data_result);
    assert!(
        matches!(terminal_id_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn terminal_id_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("terminal_id");
    entity_data_file.push("TerminalIdTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("terminal_id01"), Value::str("terminal_id02"), Value::str("terminal_id03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_TERMINAL_ID_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_TERMINAL_ID_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_TERMINAL_ID_ENTID")) {
