...
#[test]
fn ecom_parameter_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.ecom_parameter(Value::Noval);
    assert_eq!(ent.get_name(), "ecom_parameter");
}

#[test]
fn ecom_parameter_entity_stream() {
    ...
    let plainent = plainsdk.ecom_parameter(Value::Noval);
...
#[test]
fn ecom_parameter_entity_basic() {
    let setup = ecom_parameter_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("ecom_parameter.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let ecom_parameter_ref01_ent = client.ecom_parameter(Value::Noval);
    let ecom_parameter_ref01_data = to_map(&getp(
        &getpath(&["new", "ecom_parameter"], &setup.data),
        "ecom_parameter_ref01",
    ));

    let ecom_parameter_ref01_data_result = ecom_parameter_ref01_ent
        .create(ecom_parameter_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let ecom_parameter_ref01_data = to_map(&ecom_parameter_ref01_data_result);
    assert!(
        matches!(ecom_parameter_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn ecom_parameter_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("ecom_parameter");
    entity_data_file.push("EcomParameterTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("ecom_parameter01"), Value::str("ecom_parameter02"), Value::str("ecom_parameter03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_ECOM_PARAMETER_ENTID")) {
