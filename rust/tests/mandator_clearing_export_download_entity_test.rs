...
#[test]
fn mandator_clearing_export_download_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.mandator_clearing_export_download(Value::Noval);
    assert_eq!(ent.get_name(), "mandator_clearing_export_download");
}

#[test]
fn mandator_clearing_export_download_entity_stream() {
    ...
    let plainent = plainsdk.mandator_clearing_export_download(Value::Noval);
...
#[test]
fn mandator_clearing_export_download_entity_basic() {
    let setup = mandator_clearing_export_download_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create", "load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("mandator_clearing_export_download.{}", op), mode);
        if skip {
    ...
    // CREATE
    let mandator_clearing_export_download_ref01_ent = client.mandator_clearing_export_download(Value::Noval);
    let mandator_clearing_export_download_ref01_data = to_map(&getp(
        &getpath(&["new", "mandator_clearing_export_download"], &setup.data),
...
fn mandator_clearing_export_download_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("mandator_clearing_export_download");
    entity_data_file.push("MandatorClearingExportDownloadTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("mandator_clearing_export_download01"), Value::str("mandator_clearing_export_download02"), Value::str("mandator_clearing_export_download03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
