...
#[test]
fn introduce_mandator_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.introduce_mandator(Value::Noval);
    assert_eq!(ent.get_name(), "introduce_mandator");
}

#[test]
fn introduce_mandator_entity_stream() {
    ...
    let plainent = plainsdk.introduce_mandator(Value::Noval);
...
#[test]
fn introduce_mandator_entity_basic() {
    let setup = introduce_mandator_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("introduce_mandator.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_INTRODUCE_MANDATOR_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let introduce_mandator_ref01_ent = client.introduce_mandator(Value::Noval);
    let introduce_mandator_ref01_data = to_map(&getp(
        &getpath(&["new", "introduce_mandator"], &setup.data),
        "introduce_mandator_ref01",
    ));

    let introduce_mandator_ref01_data_result = introduce_mandator_ref01_ent
        .create(introduce_mandator_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let introduce_mandator_ref01_data = to_map(&introduce_mandator_ref01_data_result);
    assert!(
        matches!(introduce_mandator_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn introduce_mandator_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("introduce_mandator");
    entity_data_file.push("IntroduceMandatorTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("introduce_mandator01"), Value::str("introduce_mandator02"), Value::str("introduce_mandator03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_INTRODUCE_MANDATOR_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_INTRODUCE_MANDATOR_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_INTRODUCE_MANDATOR_ENTID")) {
