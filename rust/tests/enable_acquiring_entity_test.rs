...
#[test]
fn enable_acquiring_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.enable_acquiring(Value::Noval);
    assert_eq!(ent.get_name(), "enable_acquiring");
}

#[test]
fn enable_acquiring_entity_stream() {
    ...
    let plainent = plainsdk.enable_acquiring(Value::Noval);
...
#[test]
fn enable_acquiring_entity_basic() {
    let setup = enable_acquiring_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("enable_acquiring.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let enable_acquiring_ref01_ent = client.enable_acquiring(Value::Noval);
    let enable_acquiring_ref01_data = to_map(&getp(
        &getpath(&["new", "enable_acquiring"], &setup.data),
        "enable_acquiring_ref01",
    ));

    let enable_acquiring_ref01_data_result = enable_acquiring_ref01_ent
        .create(enable_acquiring_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let enable_acquiring_ref01_data = to_map(&enable_acquiring_ref01_data_result);
    assert!(
        matches!(enable_acquiring_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn enable_acquiring_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("enable_acquiring");
    entity_data_file.push("EnableAcquiringTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("enable_acquiring01"), Value::str("enable_acquiring02"), Value::str("enable_acquiring03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID")) {
