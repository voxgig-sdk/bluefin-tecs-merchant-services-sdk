...
fn emv_data_entity_stream() {
    ...
    let plainent = plainsdk.emv_data(Value::Noval);
...
fn emv_data_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_EMV_DATA_ENTID JSON to run live");
        return;
    }
    ...
    let emv_data_ref01_data = to_map(&emv_data_ref01_data_result);
    assert!(
...
fn emv_data_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_EMV_DATA_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_EMV_DATA_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_EMV_DATA_ENTID")) {
