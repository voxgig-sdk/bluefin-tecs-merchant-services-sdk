...
#[test]
fn create_product_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.create_product(Value::Noval);
    assert_eq!(ent.get_name(), "create_product");
}

#[test]
fn create_product_entity_stream() {
    ...
    let plainent = plainsdk.create_product(Value::Noval);
...
#[test]
fn create_product_entity_basic() {
    let setup = create_product_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("create_product.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_CREATE_PRODUCT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let create_product_ref01_ent = client.create_product(Value::Noval);
    let create_product_ref01_data = to_map(&getp(
        &getpath(&["new", "create_product"], &setup.data),
        "create_product_ref01",
    ));

    let create_product_ref01_data_result = create_product_ref01_ent
        .create(create_product_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let create_product_ref01_data = to_map(&create_product_ref01_data_result);
    assert!(
        matches!(create_product_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn create_product_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("create_product");
    entity_data_file.push("CreateProductTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("create_product01"), Value::str("create_product02"), Value::str("create_product03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_CREATE_PRODUCT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_CREATE_PRODUCT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_CREATE_PRODUCT_ENTID")) {
