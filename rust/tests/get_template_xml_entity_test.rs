...
#[test]
fn get_template_xml_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.get_template_xml(Value::Noval);
    assert_eq!(ent.get_name(), "get_template_xml");
}

#[test]
fn get_template_xml_entity_stream() {
    ...
    let plainent = plainsdk.get_template_xml(Value::Noval);
...
#[test]
fn get_template_xml_entity_basic() {
    let setup = get_template_xml_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("get_template_xml.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let get_template_xml_ref01_ent = client.get_template_xml(Value::Noval);
    let get_template_xml_ref01_data = to_map(&getp(
        &getpath(&["new", "get_template_xml"], &setup.data),
        "get_template_xml_ref01",
    ));

    let get_template_xml_ref01_data_result = get_template_xml_ref01_ent
        .create(get_template_xml_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let get_template_xml_ref01_data = to_map(&get_template_xml_ref01_data_result);
    assert!(
        matches!(get_template_xml_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn get_template_xml_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("get_template_xml");
    entity_data_file.push("GetTemplateXmlTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("get_template_xml01"), Value::str("get_template_xml02"), Value::str("get_template_xml03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTSERVICES_TEST_GET_TEMPLATE_XML_ENTID")) {
