...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinTecsMerchantServicesError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinTecsMerchantServicesSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinTecsMerchantServicesError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinTecsMerchantServicesError::new("fetch_body", &format!("{}", e)))?;
