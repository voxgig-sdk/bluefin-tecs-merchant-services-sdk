import '../BluefinTecsMerchantServicesError.dart';
import '../Result.dart';
...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'BluefinTecsMerchantServicesSDK: ' + opname.toString() + ': ' + errmsg(err);

  BluefinTecsMerchantServicesError sdkerr;
  if (err is BluefinTecsMerchantServicesError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = BluefinTecsMerchantServicesError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
