...
void tests() {
  describe('TerminalIdEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.TerminalId();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'terminal_id.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['terminal_id01', 'terminal_id02', 'terminal_id03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_TERMINAL_ID_ENTID'];
