...
void tests() {
  describe('MandatorClearingExportSummaryEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantServicesSDK.test();
      final ent = testsdk.MandatorClearingExportSummary();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MandatorClearingExportSummary();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'mandator_clearing_export_summary.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final mandator_clearing_export_summary_ref01_ent = client.MandatorClearingExportSummary();
      dynamic mandator_clearing_export_summary_ref01_data = setup['data']['new']['mandator_clearing_export_summary']['mandator_clearing_export_summary_ref01'];
      ...
      ok(null != mandator_clearing_export_summary_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['mandator_clearing_export_summary01', 'mandator_clearing_export_summary02', 'mandator_clearing_export_summary03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_SUMMARY_ENTID'];
