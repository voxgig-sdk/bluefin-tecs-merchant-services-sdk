...
void tests() {
  describe('PaymentManualEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.PaymentManual();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'payment_manual.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['payment_manual01', 'payment_manual02', 'payment_manual03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PAYMENT_MANUAL_ENTID'];
