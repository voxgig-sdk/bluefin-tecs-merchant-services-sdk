...
void tests() {
  describe('CreateProductEntity', () {
    test('instance', (t) async {
      ...
      final ent = testsdk.CreateProduct();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.CreateProduct();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'create_product.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final create_product_ref01_ent = client.CreateProduct();
      dynamic create_product_ref01_data = setup['data']['new']['create_product']['create_product_ref01'];

      create_product_ref01_data = await create_product_ref01_ent.create(create_product_ref01_data);
      ok(null != create_product_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['create_product01', 'create_product02', 'create_product03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_CREATE_PRODUCT_ENTID'];
