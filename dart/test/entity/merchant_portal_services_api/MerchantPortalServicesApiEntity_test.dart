...
void tests() {
  describe('MerchantPortalServicesApiEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantServicesSDK.test();
      final ent = testsdk.MerchantPortalServicesApi();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MerchantPortalServicesApi();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'merchant_portal_services_api.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final merchant_portal_services_api_ref01_ent = client.MerchantPortalServicesApi();
      dynamic merchant_portal_services_api_ref01_data = setup['data']['new']['merchant_portal_services_api']['merchant_portal_services_api_ref01'];
      ...
      ok(null != merchant_portal_services_api_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['merchant_portal_services_api01', 'merchant_portal_services_api02', 'merchant_portal_services_api03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MERCHANT_PORTAL_SERVICES_API_ENTID'];
