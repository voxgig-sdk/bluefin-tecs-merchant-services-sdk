...
void tests() {
  describe('PreAuthTransactionCompletionEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantServicesSDK.test();
      final ent = testsdk.PreAuthTransactionCompletion();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.PreAuthTransactionCompletion();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'pre_auth_transaction_completion.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['pre_auth_transaction_completion01', 'pre_auth_transaction_completion02', 'pre_auth_transaction_completion03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_PRE_AUTH_TRANSACTION_COMPLETION_ENTID'];
