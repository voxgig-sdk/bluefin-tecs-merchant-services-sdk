...
void tests() {
  describe('EcrDataEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.EcrData();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'ecr_data.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['ecr_data01', 'ecr_data02', 'ecr_data03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_ECR_DATA_ENTID'];
