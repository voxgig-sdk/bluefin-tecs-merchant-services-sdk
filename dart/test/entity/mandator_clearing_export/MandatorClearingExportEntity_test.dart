...
void tests() {
  describe('MandatorClearingExportEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantServicesSDK.test();
      final ent = testsdk.MandatorClearingExport();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MandatorClearingExport();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'mandator_clearing_export.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final mandator_clearing_export_ref01_ent = client.MandatorClearingExport();
      dynamic mandator_clearing_export_ref01_data = setup['data']['new']['mandator_clearing_export']['mandator_clearing_export_ref01'];

      mandator_clearing_export_ref01_data = await mandator_clearing_export_ref01_ent.create(mandator_clearing_export_ref01_data);
      ok(null != mandator_clearing_export_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['mandator_clearing_export01', 'mandator_clearing_export02', 'mandator_clearing_export03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID'];
