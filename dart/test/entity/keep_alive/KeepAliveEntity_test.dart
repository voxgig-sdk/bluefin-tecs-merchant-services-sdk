...
void tests() {
  describe('KeepAliveEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.KeepAlive();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'keep_alive.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['keep_alive01', 'keep_alive02', 'keep_alive03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_KEEP_ALIVE_ENTID'];
