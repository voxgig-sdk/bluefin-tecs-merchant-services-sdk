...
void tests() {
  describe('MandatorClearingExportDownloadEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantServicesSDK.test();
      final ent = testsdk.MandatorClearingExportDownload();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MandatorClearingExportDownload();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'load']) {
        if (maybeSkipControl(t, 'entityOp', 'mandator_clearing_export_download.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['mandator_clearing_export_download01', 'mandator_clearing_export_download02', 'mandator_clearing_export_download03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_SERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID'];
