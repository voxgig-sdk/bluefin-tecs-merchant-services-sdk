...
void tests() {
  describe('MandatorClearingExportDownloadDirect', () {
    test('direct-exists', (t) async {
      final sdk = BluefinTecsMerchantServicesSDK({
        'system': {
    ...
    test('direct-load-mandator_clearing_export_download', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-mandator_clearing_export_download', true == setup['live'])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID'];
