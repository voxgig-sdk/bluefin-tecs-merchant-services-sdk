// Generated direct-call test for the mandator_clearing_export_download entity (mirrors the
...
static voxgig_value* mandator_clearing_export_download_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinTecsMerchantServicesSDK* mandator_clearing_export_download_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinTecsMerchantServicesSDK* sdk = mandator_clearing_export_download_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "id", v_str("direct01"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("public/digitalservices/mandatorClearingExportDownload/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
