...
int main(void) {
  ...
  Entity* e = bluefin_tecs_merchant_services_ecr_data(sdk, NULL);
  ...
  // the materialised items — either way every item is yielded.
  {
    ...
    Entity* se = bluefin_tecs_merchant_services_ecr_data(strsdk, NULL);
    PNError* serr = NULL;
    voxgig_value* items = ecr_data_stream(se, "list", NULL, NULL, &serr);
    ...
    Entity* pe = bluefin_tecs_merchant_services_ecr_data(plainsdk, NULL);
    PNError* perr = NULL;
    voxgig_value* pitems = ecr_data_stream(pe, "list", NULL, NULL, &perr);
