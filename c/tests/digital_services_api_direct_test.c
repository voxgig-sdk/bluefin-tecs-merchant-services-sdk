// Generated direct-call test for the digital_services_api entity (mirrors the
...
static voxgig_value* digital_services_api_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinTecsMerchantServicesSDK* digital_services_api_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(digital_services_api_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinTecsMerchantServicesSDK* sdk = digital_services_api_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("public/digitalservices/mandatorClearingExportDownload/status"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_tecs_merchant_services_digital_services_api_direct");
}
