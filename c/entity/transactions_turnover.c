// TransactionsTurnover entity client (generated — mirrors the rust Entity fragment).
...
typedef struct transactions_turnover_entity {
  ...
  bool deleted;
} transactions_turnover_entity;

typedef void (*transactions_turnover_postdone_fn)(transactions_turnover_entity* self, Context* ctx);

// Forward declarations.
static const EntityVT transactions_turnover_VT;
static const char* transactions_turnover_get_name(Entity* e);
static Entity* transactions_turnover_make(Entity* e);
static voxgig_value* transactions_turnover_data(Entity* e, voxgig_value* args);
static voxgig_value* transactions_turnover_matchv(Entity* e, voxgig_value* args);
// Ops resolve to the ENTITY (`list` to a NULL-terminated array of them).
static Entity* transactions_turnover_load(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static Entity** transactions_turnover_list(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static Entity* transactions_turnover_create(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err);
static Entity* transactions_turnover_update(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err);
static Entity* transactions_turnover_remove(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static void transactions_turnover_mark_deleted(Entity* e);
static bool transactions_turnover_deleted(Entity* e);

static Context* transactions_turnover_ent_ctx(transactions_turnover_entity* self) {
  return self->entctx;
}
...
Entity* transactions_turnover_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  transactions_turnover_entity* self = (transactions_turnover_entity*)calloc(1, sizeof(transactions_turnover_entity));
  self->base.vt = &transactions_turnover_VT;
  self->name = strdup("transactions_turnover");
  self->client = client;
...
static voxgig_value* transactions_turnover_run_op(transactions_turnover_entity* self, Context* ctx,
                                    transactions_turnover_postdone_fn post_done, PNError** err) {
...
voxgig_value* transactions_turnover_stream(Entity* e, const char* action, voxgig_value* args,
                             voxgig_value* callopts, PNError** err) {
  transactions_turnover_entity* self = (transactions_turnover_entity*)e;
  *err = NULL;
  ...
  Context* ctx = make_context_util(cs, transactions_turnover_ent_ctx(self));
...
static const char* transactions_turnover_get_name(Entity* e) {
  return ((transactions_turnover_entity*)e)->name;
}
...
static Entity* transactions_turnover_make(Entity* e) {
  ...
  return transactions_turnover_entity_new(self->client, opts);
}
...
static voxgig_value* transactions_turnover_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(transactions_turnover_ent_ctx(self), "SetData");
  }
  feature_hook_util(transactions_turnover_ent_ctx(self), "GetData");
...
static voxgig_value* transactions_turnover_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(transactions_turnover_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(transactions_turnover_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}

static Entity* transactions_turnover_load(Entity* e, voxgig_value* reqarg, voxgig_value* ctrl, PNError** err) {
  (void)e; (void)reqarg; (void)ctrl;
  *err = unsupported_op("load", "transactions_turnover");
  return NULL;
}

static Entity** transactions_turnover_list(Entity* e, voxgig_value* reqarg, voxgig_value* ctrl, PNError** err) {
  (void)e; (void)reqarg; (void)ctrl;
  *err = unsupported_op("list", "transactions_turnover");
  return NULL;
}


static void transactions_turnover_create_postdone(transactions_turnover_entity* self, Context* ctx) {
...
static Entity* transactions_turnover_create(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqdata = reqdata;
  Context* ctx = make_context_util(cs, transactions_turnover_ent_ctx(self));
  transactions_turnover_run_op(self, ctx, transactions_turnover_create_postdone, err);
  if (*err) return NULL;
...
static Entity* transactions_turnover_update(Entity* e, voxgig_value* reqarg, voxgig_value* ctrl, PNError** err) {
  (void)e; (void)reqarg; (void)ctrl;
  *err = unsupported_op("update", "transactions_turnover");
  return NULL;
}

static Entity* transactions_turnover_remove(Entity* e, voxgig_value* reqarg, voxgig_value* ctrl, PNError** err) {
  (void)e; (void)reqarg; (void)ctrl;
  *err = unsupported_op("remove", "transactions_turnover");
  return NULL;
}
...
// live record.
static void transactions_turnover_mark_deleted(Entity* e) {
  ((transactions_turnover_entity*)e)->deleted = true;
}

static bool transactions_turnover_deleted(Entity* e) {
  return ((transactions_turnover_entity*)e)->deleted;
}

static const EntityVT transactions_turnover_VT = {
  transactions_turnover_get_name,
  transactions_turnover_make,
  transactions_turnover_data,
  transactions_turnover_matchv,
  transactions_turnover_mark_deleted,
  transactions_turnover_deleted,
  transactions_turnover_load,
  transactions_turnover_list,
  transactions_turnover_create,
  transactions_turnover_update,
  transactions_turnover_remove,
};
