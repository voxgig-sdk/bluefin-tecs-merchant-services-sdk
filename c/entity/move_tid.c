// MoveTid entity client (generated — mirrors the rust Entity fragment).
...
typedef struct move_tid_entity {
...
static const char* move_tid_get_name(Entity* e);
...
Entity* move_tid_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  move_tid_entity* self = (move_tid_entity*)calloc(1, sizeof(move_tid_entity));
  self->base.vt = &move_tid_VT;
  self->name = strdup("move_tid");
  self->client = client;
...
static voxgig_value* move_tid_run_op(move_tid_entity* self, Context* ctx,
                                    move_tid_postdone_fn post_done, PNError** err) {
...
voxgig_value* move_tid_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* move_tid_make(Entity* e) {
  ...
  return move_tid_entity_new(self->client, opts);
}
...
static voxgig_value* move_tid_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(move_tid_ent_ctx(self), "SetData");
  }
  feature_hook_util(move_tid_ent_ctx(self), "GetData");
...
static voxgig_value* move_tid_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(move_tid_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(move_tid_ent_ctx(self), "GetMatch");
...
static void move_tid_create_postdone(move_tid_entity* self, Context* ctx) {
