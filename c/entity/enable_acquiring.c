// EnableAcquiring entity client (generated — mirrors the rust Entity fragment).
...
typedef struct enable_acquiring_entity {
...
static const char* enable_acquiring_get_name(Entity* e);
...
Entity* enable_acquiring_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  enable_acquiring_entity* self = (enable_acquiring_entity*)calloc(1, sizeof(enable_acquiring_entity));
  self->base.vt = &enable_acquiring_VT;
  self->name = strdup("enable_acquiring");
  self->client = client;
...
static voxgig_value* enable_acquiring_run_op(enable_acquiring_entity* self, Context* ctx,
                                    enable_acquiring_postdone_fn post_done, PNError** err) {
...
voxgig_value* enable_acquiring_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* enable_acquiring_get_name(Entity* e) {
  return ((enable_acquiring_entity*)e)->name;
}
...
static Entity* enable_acquiring_make(Entity* e) {
  ...
  return enable_acquiring_entity_new(self->client, opts);
}
...
static voxgig_value* enable_acquiring_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(enable_acquiring_ent_ctx(self), "SetData");
  }
  feature_hook_util(enable_acquiring_ent_ctx(self), "GetData");
...
static voxgig_value* enable_acquiring_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(enable_acquiring_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(enable_acquiring_ent_ctx(self), "GetMatch");
...
static void enable_acquiring_create_postdone(enable_acquiring_entity* self, Context* ctx) {
...
static const EntityVT enable_acquiring_VT = {
  ...
  enable_acquiring_update,
  enable_acquiring_remove,
};
