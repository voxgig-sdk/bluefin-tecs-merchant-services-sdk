// MandatorClearingExportSummary entity client (generated — mirrors the rust Entity fragment).
...
typedef struct mandator_clearing_export_summary_entity {
...
static const char* mandator_clearing_export_summary_get_name(Entity* e);
...
Entity* mandator_clearing_export_summary_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  mandator_clearing_export_summary_entity* self = (mandator_clearing_export_summary_entity*)calloc(1, sizeof(mandator_clearing_export_summary_entity));
  self->base.vt = &mandator_clearing_export_summary_VT;
  self->name = strdup("mandator_clearing_export_summary");
  self->client = client;
...
static voxgig_value* mandator_clearing_export_summary_run_op(mandator_clearing_export_summary_entity* self, Context* ctx,
                                    mandator_clearing_export_summary_postdone_fn post_done, PNError** err) {
...
voxgig_value* mandator_clearing_export_summary_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* mandator_clearing_export_summary_get_name(Entity* e) {
  return ((mandator_clearing_export_summary_entity*)e)->name;
}
...
static Entity* mandator_clearing_export_summary_make(Entity* e) {
  ...
  return mandator_clearing_export_summary_entity_new(self->client, opts);
}
...
static voxgig_value* mandator_clearing_export_summary_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(mandator_clearing_export_summary_ent_ctx(self), "SetData");
  }
  feature_hook_util(mandator_clearing_export_summary_ent_ctx(self), "GetData");
...
static voxgig_value* mandator_clearing_export_summary_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(mandator_clearing_export_summary_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(mandator_clearing_export_summary_ent_ctx(self), "GetMatch");
...
static void mandator_clearing_export_summary_create_postdone(mandator_clearing_export_summary_entity* self, Context* ctx) {
...
static const EntityVT mandator_clearing_export_summary_VT = {
  mandator_clearing_export_summary_get_name,
  ...
  mandator_clearing_export_summary_update,
  mandator_clearing_export_summary_remove,
};
