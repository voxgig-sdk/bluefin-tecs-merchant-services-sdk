// TransactionHistory entity client (generated — mirrors the rust Entity fragment).
...
typedef struct transaction_history_entity {
...
static const char* transaction_history_get_name(Entity* e);
...
Entity* transaction_history_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  transaction_history_entity* self = (transaction_history_entity*)calloc(1, sizeof(transaction_history_entity));
  self->base.vt = &transaction_history_VT;
  self->name = strdup("transaction_history");
  self->client = client;
...
static voxgig_value* transaction_history_run_op(transaction_history_entity* self, Context* ctx,
                                    transaction_history_postdone_fn post_done, PNError** err) {
...
voxgig_value* transaction_history_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* transaction_history_get_name(Entity* e) {
  return ((transaction_history_entity*)e)->name;
}
...
static Entity* transaction_history_make(Entity* e) {
  ...
  return transaction_history_entity_new(self->client, opts);
}
...
static voxgig_value* transaction_history_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(transaction_history_ent_ctx(self), "SetData");
  }
  feature_hook_util(transaction_history_ent_ctx(self), "GetData");
...
static voxgig_value* transaction_history_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(transaction_history_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(transaction_history_ent_ctx(self), "GetMatch");
...
static void transaction_history_create_postdone(transaction_history_entity* self, Context* ctx) {
