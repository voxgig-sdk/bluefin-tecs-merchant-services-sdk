// PaymentManual entity client (generated — mirrors the rust Entity fragment).
...
typedef struct payment_manual_entity {
...
static const char* payment_manual_get_name(Entity* e);
...
Entity* payment_manual_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  payment_manual_entity* self = (payment_manual_entity*)calloc(1, sizeof(payment_manual_entity));
  self->base.vt = &payment_manual_VT;
  self->name = strdup("payment_manual");
  self->client = client;
...
static voxgig_value* payment_manual_run_op(payment_manual_entity* self, Context* ctx,
                                    payment_manual_postdone_fn post_done, PNError** err) {
...
voxgig_value* payment_manual_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* payment_manual_get_name(Entity* e) {
  return ((payment_manual_entity*)e)->name;
}
...
static Entity* payment_manual_make(Entity* e) {
  ...
  return payment_manual_entity_new(self->client, opts);
}
...
static voxgig_value* payment_manual_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(payment_manual_ent_ctx(self), "SetData");
  }
  feature_hook_util(payment_manual_ent_ctx(self), "GetData");
...
static voxgig_value* payment_manual_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(payment_manual_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(payment_manual_ent_ctx(self), "GetMatch");
...
static void payment_manual_create_postdone(payment_manual_entity* self, Context* ctx) {
