// PaymentSred entity client (generated — mirrors the rust Entity fragment).
...
typedef struct payment_sred_entity {
...
static const char* payment_sred_get_name(Entity* e);
...
Entity* payment_sred_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  payment_sred_entity* self = (payment_sred_entity*)calloc(1, sizeof(payment_sred_entity));
  self->base.vt = &payment_sred_VT;
  self->name = strdup("payment_sred");
  self->client = client;
...
static voxgig_value* payment_sred_run_op(payment_sred_entity* self, Context* ctx,
                                    payment_sred_postdone_fn post_done, PNError** err) {
...
voxgig_value* payment_sred_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* payment_sred_get_name(Entity* e) {
  return ((payment_sred_entity*)e)->name;
}
...
static Entity* payment_sred_make(Entity* e) {
  ...
  return payment_sred_entity_new(self->client, opts);
}
...
static voxgig_value* payment_sred_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(payment_sred_ent_ctx(self), "SetData");
  }
  feature_hook_util(payment_sred_ent_ctx(self), "GetData");
...
static voxgig_value* payment_sred_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(payment_sred_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(payment_sred_ent_ctx(self), "GetMatch");
...
static void payment_sred_create_postdone(payment_sred_entity* self, Context* ctx) {
