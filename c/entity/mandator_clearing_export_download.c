// MandatorClearingExportDownload entity client (generated — mirrors the rust Entity fragment).
...
typedef struct mandator_clearing_export_download_entity {
...
static const char* mandator_clearing_export_download_get_name(Entity* e);
...
Entity* mandator_clearing_export_download_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  mandator_clearing_export_download_entity* self = (mandator_clearing_export_download_entity*)calloc(1, sizeof(mandator_clearing_export_download_entity));
  self->base.vt = &mandator_clearing_export_download_VT;
  self->name = strdup("mandator_clearing_export_download");
  self->client = client;
...
static voxgig_value* mandator_clearing_export_download_run_op(mandator_clearing_export_download_entity* self, Context* ctx,
                                    mandator_clearing_export_download_postdone_fn post_done, PNError** err) {
...
voxgig_value* mandator_clearing_export_download_stream(Entity* e, const char* action, voxgig_value* args,
...
static const char* mandator_clearing_export_download_get_name(Entity* e) {
  return ((mandator_clearing_export_download_entity*)e)->name;
}
...
static Entity* mandator_clearing_export_download_make(Entity* e) {
  ...
  return mandator_clearing_export_download_entity_new(self->client, opts);
}
...
static voxgig_value* mandator_clearing_export_download_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(mandator_clearing_export_download_ent_ctx(self), "SetData");
  }
  feature_hook_util(mandator_clearing_export_download_ent_ctx(self), "GetData");
...
static voxgig_value* mandator_clearing_export_download_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(mandator_clearing_export_download_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(mandator_clearing_export_download_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void mandator_clearing_export_download_load_postdone(mandator_clearing_export_download_entity* self, Context* ctx) {
...
static void mandator_clearing_export_download_create_postdone(mandator_clearing_export_download_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
...
static const EntityVT mandator_clearing_export_download_VT = {
  mandator_clearing_export_download_get_name,
  ...
  mandator_clearing_export_download_update,
  mandator_clearing_export_download_remove,
};
