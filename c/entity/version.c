// Version entity client (generated — mirrors the rust Entity fragment).
...
static const char* version_get_name(Entity* e);
...
Entity* version_entity_new(BluefinTecsMerchantServicesSDK* client, voxgig_value* entopts) {
  ...
  version_entity* self = (version_entity*)calloc(1, sizeof(version_entity));
  self->base.vt = &version_VT;
  self->name = strdup("version");
  self->client = client;
...
static voxgig_value* version_run_op(version_entity* self, Context* ctx,
                                    version_postdone_fn post_done, PNError** err) {
...
voxgig_value* version_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* version_make(Entity* e) {
  ...
  return version_entity_new(self->client, opts);
}
...
static voxgig_value* version_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(version_ent_ctx(self), "SetData");
  }
  feature_hook_util(version_ent_ctx(self), "GetData");
...
static voxgig_value* version_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(version_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(version_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void version_load_postdone(version_entity* self, Context* ctx) {
