# BluefinTecsMerchantServices SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinTecsMerchantServicesHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinTecsMerchantServicesHelpers::gpath($options, 'auth.prefix');
