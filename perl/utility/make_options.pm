# BluefinTecsMerchantServices SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinTecsMerchantServicesHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinTecsMerchantServicesHelpers::gp($options, 'extend');
  ...
  my @featureorder;
  my $feature_raw = BluefinTecsMerchantServicesHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinTecsMerchantServicesHelpers::gp($entry, 'name');
  ...
  BluefinTecsMerchantServicesHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinTecsMerchantServicesHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinTecsMerchantServicesConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinTecsMerchantServicesHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinTecsMerchantServicesHelpers::to_map(
      BluefinTecsMerchantServicesHelpers::gp($opts, 'feature')) || {};
