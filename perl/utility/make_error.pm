...
$REGISTRY{make_error} = sub {
  my ($ctx, $err) = @_;
  if (!defined $ctx) {
    $ctx = BluefinTecsMerchantServicesContext->new({}, undef);
  }
  my $op = $ctx->{op} || BluefinTecsMerchantServicesOperation->new({});
  my $opname = $op->{name};
  $opname = 'unknown operation' if !defined $opname || '' eq $opname || '_' eq $opname;

  my $result = $ctx->{result} || BluefinTecsMerchantServicesResult->new({});
  $result->{ok} = 0;

  $err = $result->{err} if !defined $err;
  $err = $ctx->make_error('unknown', 'unknown error') if !defined $err;

  my $errmsg = (Scalar::Util::blessed($err) && $err->isa('BluefinTecsMerchantServicesError'))
    ? $err->{msg} : "$err";
  $errmsg =~ s/\s+\z//;
  my $msg = "BluefinTecsMerchantServicesSDK: $opname: $errmsg";
  ...
  my $sdk_err = BluefinTecsMerchantServicesError->new('', $msg, $ctx);
