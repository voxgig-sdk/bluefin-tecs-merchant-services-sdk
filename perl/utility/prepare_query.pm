# BluefinTecsMerchantServices SDK utility: prepare_query
...
$REGISTRY{prepare_query} = sub {
  ...
  my $params = [];
  if ($point) {
    my $p = BluefinTecsMerchantServicesHelpers::gp($point, 'params');
  ...
  if ($items) {
    for my $item (@$items) {
      my ($key, $val) = @$item;
      next unless BluefinTecsMerchantServicesHelpers::rb_truthy($val) && defined $key && !ref $key;
