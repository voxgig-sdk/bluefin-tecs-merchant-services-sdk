# BluefinTecsMerchantServices SDK GetTemplateXml entity
...
sub make {
  ...
  return GetTemplateXmlEntity->new($self->{_client}, $opts);
}
...
# Returns the current GetTemplateXml data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of GetTemplateXml fields).
sub match_get {
  my ($self) = @_;
...
# Create a new GetTemplateXml.
#
# reqdata: body data hashref (GetTemplateXmlCreateData shape). ctrl: optional
# per-call control. Returns the created GetTemplateXml data (hashref); dies
