# BluefinTecsMerchantServices SDK EcDataEcom entity
...
sub make {
  ...
  return EcDataEcomEntity->new($self->{_client}, $opts);
}
...
# Returns the current EcDataEcom data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of EcDataEcom fields).
sub match_get {
  my ($self) = @_;
...
# Create a new EcDataEcom.
#
# reqdata: body data hashref (EcDataEcomCreateData shape). ctrl: optional
# per-call control. Returns the created EcDataEcom data (hashref); dies
