# BluefinTecsMerchantServices SDK TerminalId entity
...
sub make {
  ...
  return TerminalIdEntity->new($self->{_client}, $opts);
}
...
# Returns the current TerminalId data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TerminalId fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TerminalId.
#
# reqdata: body data hashref (TerminalIdCreateData shape). ctrl: optional
# per-call control. Returns the created TerminalId data (hashref); dies
