# BluefinTecsMerchantServices SDK EcomParameter entity
...
sub make {
  ...
  return EcomParameterEntity->new($self->{_client}, $opts);
}
...
# Returns the current EcomParameter data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of EcomParameter fields).
sub match_get {
  my ($self) = @_;
...
# Create a new EcomParameter.
#
# reqdata: body data hashref (EcomParameterCreateData shape). ctrl: optional
# per-call control. Returns the created EcomParameter data (hashref); dies
