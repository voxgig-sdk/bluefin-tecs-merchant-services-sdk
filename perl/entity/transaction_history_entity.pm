# BluefinTecsMerchantServices SDK TransactionHistory entity
...
package TransactionHistoryEntity;
...
sub make {
  ...
  return TransactionHistoryEntity->new($self->{_client}, $opts);
}
...
# Returns the current TransactionHistory data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TransactionHistory fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (TransactionHistoryCreateData shape). ctrl: optional
# per-call control. Returns the created TransactionHistory data (hashref); dies
