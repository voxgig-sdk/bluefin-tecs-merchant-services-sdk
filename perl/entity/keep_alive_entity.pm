# BluefinTecsMerchantServices SDK KeepAlive entity
...
sub make {
  ...
  return KeepAliveEntity->new($self->{_client}, $opts);
}
...
# Returns the current KeepAlive data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of KeepAlive fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (KeepAliveCreateData shape). ctrl: optional
# per-call control. Returns the created KeepAlive data (hashref); dies
