# BluefinTecsMerchantServices SDK GetMerchantContractNumber entity
...
package GetMerchantContractNumberEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'get_merchant_contract_number',
    _client => $client,
...
sub make {
  ...
  return GetMerchantContractNumberEntity->new($self->{_client}, $opts);
}
...
# Returns the current GetMerchantContractNumber data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of GetMerchantContractNumber fields).
sub match_get {
  my ($self) = @_;
...
# Create a new GetMerchantContractNumber.
#
# reqdata: body data hashref (GetMerchantContractNumberCreateData shape). ctrl: optional
# per-call control. Returns the created GetMerchantContractNumber data (hashref); dies
