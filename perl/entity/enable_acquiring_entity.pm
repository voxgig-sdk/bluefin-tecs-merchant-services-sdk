# BluefinTecsMerchantServices SDK EnableAcquiring entity
...
sub make {
  ...
  return EnableAcquiringEntity->new($self->{_client}, $opts);
}
...
# Returns the current EnableAcquiring data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of EnableAcquiring fields).
sub match_get {
  my ($self) = @_;
...
# Create a new EnableAcquiring.
#
# reqdata: body data hashref (EnableAcquiringCreateData shape). ctrl: optional
# per-call control. Returns the created EnableAcquiring data (hashref); dies
