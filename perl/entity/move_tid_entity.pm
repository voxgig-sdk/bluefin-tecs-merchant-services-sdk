# BluefinTecsMerchantServices SDK MoveTid entity
...
sub make {
  ...
  return MoveTidEntity->new($self->{_client}, $opts);
}
...
# Returns the current MoveTid data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of MoveTid fields).
sub match_get {
  my ($self) = @_;
...
# Create a new MoveTid.
#
# reqdata: body data hashref (MoveTidCreateData shape). ctrl: optional
# per-call control. Returns the created MoveTid data (hashref); dies
