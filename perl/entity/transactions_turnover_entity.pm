# BluefinTecsMerchantServices SDK TransactionsTurnover entity
...
package TransactionsTurnoverEntity;
...
sub make {
  ...
  return TransactionsTurnoverEntity->new($self->{_client}, $opts);
}
...
# Returns the current TransactionsTurnover data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TransactionsTurnover fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TransactionsTurnover.
#
# reqdata: body data hashref (TransactionsTurnoverCreateData shape). ctrl: optional
# per-call control. Returns the created TransactionsTurnover data (hashref); dies
