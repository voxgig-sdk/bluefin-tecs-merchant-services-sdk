# BluefinTecsMerchantServices SDK PreAuthTransactionCompletion entity
...
package PreAuthTransactionCompletionEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'pre_auth_transaction_completion',
    _client => $client,
...
sub make {
  ...
  return PreAuthTransactionCompletionEntity->new($self->{_client}, $opts);
}
...
# Returns the current PreAuthTransactionCompletion data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of PreAuthTransactionCompletion fields).
sub match_get {
  my ($self) = @_;
...
# Create a new PreAuthTransactionCompletion.
#
# reqdata: body data hashref (PreAuthTransactionCompletionCreateData shape). ctrl: optional
# per-call control. Returns the created PreAuthTransactionCompletion data (hashref); dies
