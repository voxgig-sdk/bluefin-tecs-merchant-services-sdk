# BluefinTecsMerchantServices SDK DeactivateTerminal entity
...
package DeactivateTerminalEntity;
...
sub make {
  ...
  return DeactivateTerminalEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeactivateTerminal data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeactivateTerminal fields).
sub match_get {
  my ($self) = @_;
...
# Create a new DeactivateTerminal.
#
# reqdata: body data hashref (DeactivateTerminalCreateData shape). ctrl: optional
# per-call control. Returns the created DeactivateTerminal data (hashref); dies
