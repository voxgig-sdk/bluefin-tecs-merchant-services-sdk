...
package MandatorClearingExportDownloadEntity;
...
sub new {
  ...
  elsif (BluefinTecsMerchantServicesHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
  ...
  my $self = bless {
    _name => 'mandator_clearing_export_download',
    _client => $client,
...
sub make {
  ...
  return MandatorClearingExportDownloadEntity->new($self->{_client}, $opts);
}
...
sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinTecsMerchantServicesHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current MandatorClearingExportDownload data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinTecsMerchantServicesHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of MandatorClearingExportDownload fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; MandatorClearingExportDownloadLoadMatch
...
# Returns the loaded MandatorClearingExportDownload data (hashref); dies with BluefinTecsMerchantServicesError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = BluefinTecsMerchantServicesHelpers::to_map(
...
#
# reqdata: body data hashref (MandatorClearingExportDownloadCreateData shape). ctrl: optional
# per-call control. Returns the created MandatorClearingExportDownload data (hashref); dies
# with BluefinTecsMerchantServicesError on failure.
sub create {
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinTecsMerchantServicesHelpers::to_map($callopts) || {};
  my $signal = BluefinTecsMerchantServicesHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinTecsMerchantServicesHelpers::to_map(
    BluefinTecsMerchantServicesHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinTecsMerchantServicesHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinTecsMerchantServicesHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinTecsMerchantServicesHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
