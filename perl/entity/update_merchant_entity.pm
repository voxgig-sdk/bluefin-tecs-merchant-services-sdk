# BluefinTecsMerchantServices SDK UpdateMerchant entity
...
sub make {
  ...
  return UpdateMerchantEntity->new($self->{_client}, $opts);
}
...
# Returns the current UpdateMerchant data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of UpdateMerchant fields).
sub match_get {
  my ($self) = @_;
...
# Create a new UpdateMerchant.
#
# reqdata: body data hashref (UpdateMerchantCreateData shape). ctrl: optional
# per-call control. Returns the created UpdateMerchant data (hashref); dies
