# BluefinTecsMerchantServices SDK IntroduceMandator entity
...
package IntroduceMandatorEntity;
...
sub make {
  ...
  return IntroduceMandatorEntity->new($self->{_client}, $opts);
}
...
# Returns the current IntroduceMandator data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of IntroduceMandator fields).
sub match_get {
  my ($self) = @_;
...
# Create a new IntroduceMandator.
#
# reqdata: body data hashref (IntroduceMandatorCreateData shape). ctrl: optional
# per-call control. Returns the created IntroduceMandator data (hashref); dies
