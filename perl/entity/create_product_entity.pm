# BluefinTecsMerchantServices SDK CreateProduct entity
...
sub make {
  ...
  return CreateProductEntity->new($self->{_client}, $opts);
}
...
# Returns the current CreateProduct data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of CreateProduct fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (CreateProductCreateData shape). ctrl: optional
# per-call control. Returns the created CreateProduct data (hashref); dies
