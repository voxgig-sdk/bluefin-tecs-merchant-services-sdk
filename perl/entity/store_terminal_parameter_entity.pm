# BluefinTecsMerchantServices SDK StoreTerminalParameter entity
...
package StoreTerminalParameterEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'store_terminal_parameter',
    _client => $client,
...
sub make {
  ...
  return StoreTerminalParameterEntity->new($self->{_client}, $opts);
}
...
# Returns the current StoreTerminalParameter data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of StoreTerminalParameter fields).
sub match_get {
  my ($self) = @_;
...
# Create a new StoreTerminalParameter.
#
# reqdata: body data hashref (StoreTerminalParameterCreateData shape). ctrl: optional
# per-call control. Returns the created StoreTerminalParameter data (hashref); dies
