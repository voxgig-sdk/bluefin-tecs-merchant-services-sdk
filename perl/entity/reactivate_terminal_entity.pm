# BluefinTecsMerchantServices SDK ReactivateTerminal entity
...
package ReactivateTerminalEntity;
...
sub make {
  ...
  return ReactivateTerminalEntity->new($self->{_client}, $opts);
}
...
# Returns the current ReactivateTerminal data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ReactivateTerminal fields).
sub match_get {
  my ($self) = @_;
...
# Create a new ReactivateTerminal.
#
# reqdata: body data hashref (ReactivateTerminalCreateData shape). ctrl: optional
# per-call control. Returns the created ReactivateTerminal data (hashref); dies
