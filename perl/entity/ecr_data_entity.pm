# BluefinTecsMerchantServices SDK EcrData entity
...
sub make {
  ...
  return EcrDataEntity->new($self->{_client}, $opts);
}
...
# Returns the current EcrData data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of EcrData fields).
sub match_get {
  my ($self) = @_;
...
# Create a new EcrData.
#
# reqdata: body data hashref (EcrDataCreateData shape). ctrl: optional
# per-call control. Returns the created EcrData data (hashref); dies
