# BluefinTecsMerchantServices SDK CancelTransaction entity
...
package CancelTransactionEntity;
...
sub make {
  ...
  return CancelTransactionEntity->new($self->{_client}, $opts);
}
...
# Returns the current CancelTransaction data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of CancelTransaction fields).
sub match_get {
  my ($self) = @_;
...
# Create a new CancelTransaction.
#
# reqdata: body data hashref (CancelTransactionCreateData shape). ctrl: optional
# per-call control. Returns the created CancelTransaction data (hashref); dies
