# BluefinTecsMerchantServices SDK StatusTransaction entity
...
package StatusTransactionEntity;
...
sub make {
  ...
  return StatusTransactionEntity->new($self->{_client}, $opts);
}
...
# Returns the current StatusTransaction data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of StatusTransaction fields).
sub match_get {
  my ($self) = @_;
...
# Create a new StatusTransaction.
#
# reqdata: body data hashref (StatusTransactionCreateData shape). ctrl: optional
# per-call control. Returns the created StatusTransaction data (hashref); dies
