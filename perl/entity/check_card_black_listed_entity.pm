# BluefinTecsMerchantServices SDK CheckCardBlackListed entity
...
package CheckCardBlackListedEntity;
...
sub make {
  ...
  return CheckCardBlackListedEntity->new($self->{_client}, $opts);
}
...
# Returns the current CheckCardBlackListed data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of CheckCardBlackListed fields).
sub match_get {
  my ($self) = @_;
...
# Create a new CheckCardBlackListed.
#
# reqdata: body data hashref (CheckCardBlackListedCreateData shape). ctrl: optional
# per-call control. Returns the created CheckCardBlackListed data (hashref); dies
