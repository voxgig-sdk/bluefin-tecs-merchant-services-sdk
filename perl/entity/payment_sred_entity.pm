# BluefinTecsMerchantServices SDK PaymentSred entity
...
sub make {
  ...
  return PaymentSredEntity->new($self->{_client}, $opts);
}
...
# Returns the current PaymentSred data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of PaymentSred fields).
sub match_get {
  my ($self) = @_;
...
# Create a new PaymentSred.
#
# reqdata: body data hashref (PaymentSredCreateData shape). ctrl: optional
# per-call control. Returns the created PaymentSred data (hashref); dies
