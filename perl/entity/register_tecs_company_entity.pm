# BluefinTecsMerchantServices SDK RegisterTecsCompany entity
...
package RegisterTecsCompanyEntity;
...
sub make {
  ...
  return RegisterTecsCompanyEntity->new($self->{_client}, $opts);
}
...
# Returns the current RegisterTecsCompany data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of RegisterTecsCompany fields).
sub match_get {
  my ($self) = @_;
...
# Create a new RegisterTecsCompany.
#
# reqdata: body data hashref (RegisterTecsCompanyCreateData shape). ctrl: optional
# per-call control. Returns the created RegisterTecsCompany data (hashref); dies
