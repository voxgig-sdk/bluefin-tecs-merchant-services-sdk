# BluefinTecsMerchantServices SDK ListTerminal entity
...
sub make {
  ...
  return ListTerminalEntity->new($self->{_client}, $opts);
}
...
# Returns the current ListTerminal data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ListTerminal fields).
sub match_get {
  my ($self) = @_;
...
# Create a new ListTerminal.
#
# reqdata: body data hashref (ListTerminalCreateData shape). ctrl: optional
# per-call control. Returns the created ListTerminal data (hashref); dies
