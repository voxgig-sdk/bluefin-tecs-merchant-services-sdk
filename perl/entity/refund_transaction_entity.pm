# BluefinTecsMerchantServices SDK RefundTransaction entity
...
package RefundTransactionEntity;
...
sub make {
  ...
  return RefundTransactionEntity->new($self->{_client}, $opts);
}
...
# Returns the current RefundTransaction data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of RefundTransaction fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (RefundTransactionCreateData shape). ctrl: optional
# per-call control. Returns the created RefundTransaction data (hashref); dies
