# BluefinTecsMerchantServices SDK RegisterTerminal entity
...
package RegisterTerminalEntity;
...
sub make {
  ...
  return RegisterTerminalEntity->new($self->{_client}, $opts);
}
...
# Returns the current RegisterTerminal data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of RegisterTerminal fields).
sub match_get {
  my ($self) = @_;
...
# Create a new RegisterTerminal.
#
# reqdata: body data hashref (RegisterTerminalCreateData shape). ctrl: optional
# per-call control. Returns the created RegisterTerminal data (hashref); dies
