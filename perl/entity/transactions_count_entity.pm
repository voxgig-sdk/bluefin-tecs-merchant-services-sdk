# BluefinTecsMerchantServices SDK TransactionsCount entity
...
package TransactionsCountEntity;
...
sub make {
  ...
  return TransactionsCountEntity->new($self->{_client}, $opts);
}
...
# Returns the current TransactionsCount data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TransactionsCount fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TransactionsCount.
#
# reqdata: body data hashref (TransactionsCountCreateData shape). ctrl: optional
# per-call control. Returns the created TransactionsCount data (hashref); dies
