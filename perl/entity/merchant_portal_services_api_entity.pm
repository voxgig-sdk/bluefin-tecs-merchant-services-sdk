# BluefinTecsMerchantServices SDK MerchantPortalServicesApi entity
...
package MerchantPortalServicesApiEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'merchant_portal_services_api',
    _client => $client,
...
sub make {
  ...
  return MerchantPortalServicesApiEntity->new($self->{_client}, $opts);
}
...
# Returns the current MerchantPortalServicesApi data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of MerchantPortalServicesApi fields).
sub match_get {
  my ($self) = @_;
...
# Create a new MerchantPortalServicesApi.
#
# reqdata: body data hashref (MerchantPortalServicesApiCreateData shape). ctrl: optional
# per-call control. Returns the created MerchantPortalServicesApi data (hashref); dies
