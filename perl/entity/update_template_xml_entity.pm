# BluefinTecsMerchantServices SDK UpdateTemplateXml entity
...
package UpdateTemplateXmlEntity;
...
sub make {
  ...
  return UpdateTemplateXmlEntity->new($self->{_client}, $opts);
}
...
# Returns the current UpdateTemplateXml data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of UpdateTemplateXml fields).
sub match_get {
  my ($self) = @_;
...
# Create a new UpdateTemplateXml.
#
# reqdata: body data hashref (UpdateTemplateXmlCreateData shape). ctrl: optional
# per-call control. Returns the created UpdateTemplateXml data (hashref); dies
