# BluefinTecsMerchantServices SDK IntroducePackage entity
...
package IntroducePackageEntity;
...
sub make {
  ...
  return IntroducePackageEntity->new($self->{_client}, $opts);
}
...
# Returns the current IntroducePackage data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of IntroducePackage fields).
sub match_get {
  my ($self) = @_;
...
# Create a new IntroducePackage.
#
# reqdata: body data hashref (IntroducePackageCreateData shape). ctrl: optional
# per-call control. Returns the created IntroducePackage data (hashref); dies
