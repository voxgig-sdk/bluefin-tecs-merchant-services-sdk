# BluefinTecsMerchantServices SDK TransactionsCountCardBrand entity
...
package TransactionsCountCardBrandEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'transactions_count_card_brand',
    _client => $client,
...
sub make {
  ...
  return TransactionsCountCardBrandEntity->new($self->{_client}, $opts);
}
...
# Returns the current TransactionsCountCardBrand data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TransactionsCountCardBrand fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TransactionsCountCardBrand.
#
# reqdata: body data hashref (TransactionsCountCardBrandCreateData shape). ctrl: optional
# per-call control. Returns the created TransactionsCountCardBrand data (hashref); dies
