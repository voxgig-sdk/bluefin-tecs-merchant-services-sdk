# BluefinTecsMerchantServices SDK PaymentManual entity
...
sub make {
  ...
  return PaymentManualEntity->new($self->{_client}, $opts);
}
...
# Returns the current PaymentManual data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of PaymentManual fields).
sub match_get {
  my ($self) = @_;
...
# Create a new PaymentManual.
#
# reqdata: body data hashref (PaymentManualCreateData shape). ctrl: optional
# per-call control. Returns the created PaymentManual data (hashref); dies
