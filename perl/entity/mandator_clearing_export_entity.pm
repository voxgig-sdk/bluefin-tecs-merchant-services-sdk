# BluefinTecsMerchantServices SDK MandatorClearingExport entity
...
package MandatorClearingExportEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'mandator_clearing_export',
    _client => $client,
...
sub make {
  ...
  return MandatorClearingExportEntity->new($self->{_client}, $opts);
}
...
# Returns the current MandatorClearingExport data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of MandatorClearingExport fields).
sub match_get {
  my ($self) = @_;
...
# Create a new MandatorClearingExport.
#
# reqdata: body data hashref (MandatorClearingExportCreateData shape). ctrl: optional
# per-call control. Returns the created MandatorClearingExport data (hashref); dies
