# BluefinTecsMerchantServices SDK Version entity
...
sub make {
  ...
  return VersionEntity->new($self->{_client}, $opts);
}
...
# Returns the current Version data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Version fields).
sub match_get {
  my ($self) = @_;
...
# Load a single Version.
#
# reqmatch: match criteria hashref (id/query fields; VersionLoadMatch
...
# Returns the loaded Version data (hashref); dies with BluefinTecsMerchantServicesError
# on failure.
sub load {
