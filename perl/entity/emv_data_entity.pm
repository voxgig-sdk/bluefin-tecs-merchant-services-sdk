# BluefinTecsMerchantServices SDK EmvData entity
...
sub make {
  ...
  return EmvDataEntity->new($self->{_client}, $opts);
}
...
# Returns the current EmvData data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of EmvData fields).
sub match_get {
  my ($self) = @_;
...
# Create a new EmvData.
#
# reqdata: body data hashref (EmvDataCreateData shape). ctrl: optional
# per-call control. Returns the created EmvData data (hashref); dies
