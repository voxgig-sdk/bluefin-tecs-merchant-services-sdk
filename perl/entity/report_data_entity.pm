# BluefinTecsMerchantServices SDK ReportData entity
...
sub make {
  ...
  return ReportDataEntity->new($self->{_client}, $opts);
}
...
# Returns the current ReportData data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ReportData fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (ReportDataCreateData shape). ctrl: optional
# per-call control. Returns the created ReportData data (hashref); dies
