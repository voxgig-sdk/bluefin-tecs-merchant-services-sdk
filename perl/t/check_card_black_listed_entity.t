...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID JSON to run live');
  ...
  $V{check_card_black_listed_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{check_card_black_listed_ref01_data_result});
...
sub check_card_black_listed_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_CHECK_CARD_BLACK_LISTED_ENTID'});
