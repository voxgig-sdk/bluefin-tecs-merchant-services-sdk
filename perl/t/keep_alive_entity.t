...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_KEEP_ALIVE_ENTID JSON to run live');
  ...
  $V{keep_alive_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{keep_alive_ref01_data_result});
...
sub keep_alive_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_KEEP_ALIVE_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_KEEP_ALIVE_ENTID'});
