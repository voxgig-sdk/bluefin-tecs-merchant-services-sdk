#!perl
# BluefinTecsMerchantServices SDK primary utility test
#
...
#   live blessed context with BluefinTecsMerchantServicesOmni::livectx before calling a
...
my $_runner = BluefinTecsMerchantServicesOmni::make_runner($TEST_JSON, BluefinTecsMerchantServicesSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinTecsMerchantServicesBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinTecsMerchantServicesBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinTecsMerchantServicesError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinTecsMerchantServicesOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinTecsMerchantServicesOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinTecsMerchantServicesBaseFeature->new;
...
{
  my $hook_client = BluefinTecsMerchantServicesSDK->test(undef, undef);
...
{
  my $init_client = BluefinTecsMerchantServicesSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinTecsMerchantServicesSDK->new({
...
{
  my $blocked_client = BluefinTecsMerchantServicesSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinTecsMerchantServicesOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinTecsMerchantServicesSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinTecsMerchantServicesSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makePoint', sub {
  my ($view) = @_;
  my ($point, $err) = $utility->{make_point}->(BluefinTecsMerchantServicesOmni::livectx($view));
  # The corpus asserts refusals by code (`match: {out: {code: ...}}`), so
  # an error is the RESULT here, flattened to the map the corpus reads.
  return BluefinTecsMerchantServicesOmni::errify($err) if defined $err;
  return $point;
});
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinTecsMerchantServicesOmni::livectx($view);
  $ctx->{result} = BluefinTecsMerchantServicesResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinTecsMerchantServicesOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinTecsMerchantServicesOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinTecsMerchantServicesSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinTecsMerchantServicesOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinTecsMerchantServicesOmni::livectx($view));
});
