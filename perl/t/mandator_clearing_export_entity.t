...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID JSON to run live');
  ...
  $V{mandator_clearing_export_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{mandator_clearing_export_ref01_data_result});
...
sub mandator_clearing_export_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_ENTID'});
