...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID JSON to run live');
  ...
  $V{enable_acquiring_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{enable_acquiring_ref01_data_result});
...
sub enable_acquiring_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_ENABLE_ACQUIRING_ENTID'});
