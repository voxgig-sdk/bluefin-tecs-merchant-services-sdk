...

{
  ...
  my $ent = $testsdk->MandatorClearingExportDownload(undef);
  ok(defined $ent, 'mandator_clearing_export_download: create instance');
}

BASIC_FLOW: {
  my $setup = mandator_clearing_export_download_basic_setup(undef);
  my $_live = $setup->{live} ? 1 : 0;
  # Per-op sdk-test-control.json skip.
  for my $_op ('create', 'load') {
    my ($_should_skip, $_reason) = BluefinTecsMerchantServicesTestRunner::is_control_skipped(
      'entityOp', "mandator_clearing_export_download." . $_op, $_live ? 'live' : 'unit');
    if ($_should_skip) {
      note($_reason || 'skipped via sdk-test-control.json');
      pass('mandator_clearing_export_download: basic flow skipped via sdk-test-control.json');
      last BASIC_FLOW;
    }
  }
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID JSON to run live');
    pass('mandator_clearing_export_download: basic flow skipped (synthetic IDs only)');
    last BASIC_FLOW;
  }
  ...
  # CREATE
  $V{mandator_clearing_export_download_ref01_ent} = $client->MandatorClearingExportDownload(undef);
  $V{mandator_clearing_export_download_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map(BluefinTecsMerchantServicesHelpers::gp(
    BluefinTecsMerchantServicesHelpers::gpath($setup->{data}, 'new.mandator_clearing_export_download'), 'mandator_clearing_export_download_ref01'));
  ...
  $V{mandator_clearing_export_download_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{mandator_clearing_export_download_ref01_data_result});
...
sub mandator_clearing_export_download_basic_setup {
  my ($extra) = @_;
  BluefinTecsMerchantServicesTestRunner::load_env_local();

  my $entity_data_file = Cwd::abs_path(
    "$FindBin::Bin/../../.sdk/test/entity/mandator_clearing_export_download/MandatorClearingExportDownloadTestData.json");
  my $entity_data = do {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_MANDATOR_CLEARING_EXPORT_DOWNLOAD_ENTID'});
