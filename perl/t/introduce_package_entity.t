...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFIN_TECS_MERCHANT_SERVICES_TEST_INTRODUCE_PACKAGE_ENTID JSON to run live');
  ...
  $V{introduce_package_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map(ref($V{introduce_package_ref01_data_result}) && $V{introduce_package_ref01_data_result}->can('data_get') ? $V{introduce_package_ref01_data_result}->data_get : $V{introduce_package_ref01_data_result});
...
sub introduce_package_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFIN_TECS_MERCHANT_SERVICES_TEST_INTRODUCE_PACKAGE_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFIN_TECS_MERCHANT_SERVICES_TEST_INTRODUCE_PACKAGE_ENTID'});
