...
# operation pipeline. $features is a list of { name =>, options => }.
{
  ...
  sub new {
    ...
    $self->{utility} = BluefinTecsMerchantServicesUtility->new;
    ...
    for my $fspec (@$features) {
      ...
      my $f = BluefinTecsMerchantServicesFeatures::make_feature($name);
  ...
  sub op {
    ...
    my $out = eval {
      ...
      if (Scalar::Util::blessed($ctx->{out}{point})
        && $ctx->{out}{point}->isa('BluefinTecsMerchantServicesError')) {
        die $ctx->{out}{point};
      }
      ...
      $ctx->{response} = Voxgig::Struct::ismap($fetched)
        ? BluefinTecsMerchantServicesResponse->new($fetched) : undef;
    ...
    if (my $err = $@) {
      if (Scalar::Util::blessed($err) && $err->isa('BluefinTecsMerchantServicesError')) {
        $ctx->{ctrl}{err} = $err;
  ...
  sub populate_result {
    ...
    my $result = BluefinTecsMerchantServicesResult->new({});
    $ctx->{result} = $result;
...
if (has_feature('retry')) {
...

  {
    ...
    my ($server, $calls) = recording_server(sub {
      my ($n) = @_;
      return $n < 3
        ? (undef, BluefinTecsMerchantServicesError->new('boom', 'boom'))
