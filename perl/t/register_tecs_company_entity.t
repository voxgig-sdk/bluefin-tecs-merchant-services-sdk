...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID JSON to run live');
  ...
  $V{register_tecs_company_ref01_data} = BluefinTecsMerchantServicesHelpers::to_map($V{register_tecs_company_ref01_data_result});
...
sub register_tecs_company_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID'};
  ...
  my $idmap_resolved = BluefinTecsMerchantServicesHelpers::to_map($env->{'BLUEFINTECSMERCHANTSERVICES_TEST_REGISTER_TECS_COMPANY_ENTID'});
