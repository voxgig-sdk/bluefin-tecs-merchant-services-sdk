...
package BluefinTecsMerchantServicesContext;
...
sub new {
  ...
  my $gcp = \&BluefinTecsMerchantServicesHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinTecsMerchantServicesControl->new();
  ...
  $self->{data} = BluefinTecsMerchantServicesHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinTecsMerchantServicesHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinTecsMerchantServicesHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinTecsMerchantServicesHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinTecsMerchantServicesSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinTecsMerchantServicesResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinTecsMerchantServicesResponse'))
...
sub resolve_op {
  ...
  return BluefinTecsMerchantServicesOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinTecsMerchantServicesHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinTecsMerchantServicesHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinTecsMerchantServicesOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinTecsMerchantServicesError->new($code, $msg, $self);
}
