# bluefin-tecs-merchant-services-cli

boru-driven command-line client **and** interactive REPL for the BluefinTecsMerchantServices
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-tecs-merchant-services-cli)
make build

# 2. See usage (words, entities, env vars)
./bluefin-tecs-merchant-services-cli --help
...
BLUEFIN_TECS_MERCHANT_SERVICES_BASE=https://api.example.com ./bluefin-tecs-merchant-services-cli --help

# 6. No arguments -> interactive REPL
./bluefin-tecs-merchant-services-cli
bluefin-tecs-merchant-services> /help
bluefin-tecs-merchant-services> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-tecs-merchant-services-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bluefin-tecs-merchant-services-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `bluefin-tecs-merchant-services>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cancel_transaction`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bluefin-tecs-merchant-services-cli`. |
...
(`./bluefin-tecs-merchant-services-cli <expr>`) and interactively (the REPL), and expressions compose the
